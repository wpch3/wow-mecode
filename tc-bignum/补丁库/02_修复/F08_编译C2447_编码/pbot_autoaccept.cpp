...
#include "TradeData.h"          // TradeData::GetTrader (TradeData.h:41 public)
#include "World.h"
...
[[maybe_unused]] static void TryAcceptTrade(Player* bot)
{
    ...
    Player* trader = trade->GetTrader();        // TradeData.h:41 public
    if (!trader)
        return;
    ...
    // TradeHandler.cpp  HandleBeginTradeOpcode  参数没用到，可空调用
    WorldPacket dummy(CMSG_BEGIN_TRADE, 0);
    bot->GetSession()->HandleBeginTradeOpcode(dummy);   // WorldSession.h:924 public(706段)
