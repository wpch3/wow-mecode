...
protected:
    ...
    uint32 _careTimer;          // step34 
    uint32 _requestTimer;       // A37第5步 主动索要冷却
    uint8 _lastRequestType;     // A37第5步 上次要的是什么，避免连续重复
    uint32 _careChatTimer;      // step34 
    uint8  _lastMasterLevel;    // step34 
    bool   _masterWasDead;      // step34 
    bool   _careStocked;         // step34
    uint32 _careRestockTimer;    // step34
    bool Feasting() const;
