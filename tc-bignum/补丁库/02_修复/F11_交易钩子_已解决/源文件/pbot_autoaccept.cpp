...
{
    // ================================================================
    //  step57 【职责变更】这个函数不再负责"打开窗口"
    // ================================================================
    //  开窗口改由核心钩子做了 —— TradeHandler.cpp 的
    //  HandleInitiateTradeOpcode 结尾，交易建立那一刻直接调
    //  HandleBeginTradeOpcode。那是零延迟、零状态的路径。
    //
    //  【为什么不再用轮询开窗】前三次(step42/53/56)都走轮询，都没成。
    //  轮询有三个绕不过去的缺陷：
    //    1. _pbotTradeOpened 这个静态集合一旦被污染（开窗失败但
    //       m_trade 仍非空，不触发清理），该 bot 的交易【永远】不再处理，
    //       只有重启进程能恢复
    //    2. g_pbots 是纯内存的，服务器重启后为空 -> 轮不到任何 bot
    //    3. 1 秒的检查间隔，客户端可能已经放弃
    //
    //  现在这里【只保留距离自愈】—— 这是钩子做不到的，
    //  因为它需要在交易建立【之后】持续监控。
    ...
    // ---- 距离自愈 ----
    //  官方只在【发起交易时】查距离(TradeHandler.cpp:704)，
    //  建立之后不再查，靠客户端主动发 CMSG_CANCEL_TRADE。
    //  bot 没客户端不会发 -> 你走远了交易还挂着 -> m_trade 不释放
    //  -> 再点交易被 TradeHandler.cpp:611 拦下 -> "你已经在交易了"。
    //
