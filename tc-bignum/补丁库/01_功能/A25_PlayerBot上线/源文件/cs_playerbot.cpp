...
#include "Map.h"                // Map::IsDungeon / IsBattlegroundOrArena / GetInstanceId
                                //【教训】上次 Vehicle 就是漏 include 导致 C2027
...
public:
    ...
    static void SendHelp(ChatHandler* handler)
    {
        ...
        handler->SendSysMessage("|cffff0000 注意：该角色不能同时被真人登录|r");
    }
    ...
    static bool HandlePBot(ChatHandler* handler, char const* args)
    {
        ...
        if (s0 == "list" || tok[0] == "列表")
        {
            if (g_pbots.empty())
            {
                handler->SendSysMessage("|cffffff00 没有已登录的 PlayerBot|r");
                return true;
            }
        ...
        if (s0 == "despawn" || tok[0] == "下线")
        {
            ...
            handler->SendSysMessage("|cffff0000 没找到这个bot|r");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (s0 == "come" || tok[0] == "过来" || tok[0] == "召唤")
        {
            ...
            for (PBotEntry const& e : g_pbots)
            {
                ...
                {
                    handler->SendSysMessage("|cffff0000 传送失败（坐标非法或地图不可进入）|r");
                    handler->SetSentErrorMessage(true);
                    return false;
                }
        ...
        if (s0 == "auto" || tok[0] == "自动")
        {
            ...
            for (PBotEntry& e : g_pbots)
            {
                ...
                if (!applyAll)
                {
                    snprintf(buf, sizeof(buf), "|cff00ff00%s|r 现在: %s",
                             e.CharName.c_str(), PBotFlagsToText(e.AutoFlags).c_str());
                    handler->SendSysMessage(buf);
                    return true;
                }
        ...
        // ---------- accept ----------
        //  手动让bot立刻接受当前挂着的邀请（不等下一次轮询）
        //  用途：你就想马上生效，或者临时关了自动但这次想接
        if (s0 == "accept" || tok[0] == "接受")
        {
            ...
            if (!done)
            {
                handler->SendSysMessage("|cffff0000 没找到在线的bot|r");
                handler->SetSentErrorMessage(true);
                return false;
            }
