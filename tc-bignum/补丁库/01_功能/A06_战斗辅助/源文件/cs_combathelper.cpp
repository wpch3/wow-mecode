...
namespace CombatHelper
{
     ...
     */
    inline bool HasAnyRankAura(Player* p, uint32 rank1)
    {
     ...
     */
    inline int32 AuraRemainMs(Player* p, uint32 rank1)
    {
        uint32 cur = sSpellMgr->GetFirstSpellInChain(rank1);
        if (!cur)
            cur = rank1;
        ...
        while (cur && guard++ < 20)
        {
            if (AuraApplication const* app = p->GetAuraApplication(cur))
            {
    ...
    inline void PickHealTarget(Player* player, float maxDist, HealTarget& out)
    {
        ...
        auto consider = [&](Unit* u, bool tankHint)
        {
            if (!u || !u->IsAlive() || !u->IsInMap(player))
                return;
            if (player->GetExactDist2d(u) > maxDist)
                return;
            ...
            // 坦克在同血量下优先：给它 5% 的"虚拟加权"
            float weighted = tankHint ? pct - 5.0f : pct;
            float curW     = out.isTank ? out.hpPct - 5.0f : out.hpPct;
        ...
        if (grp)
        {
            for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                /*
                 * 怎么认坦克：没有现成的"角色标记"能直接读，
                 * 用两个可靠特征——防护/鲜血/熊德专精的职业，
                 * 且血上限明显高于自己（坦克堆耐力）。
                 * 认错也不影响正确性，只影响优先级微调。
                 */
                bool tankish = false;
                uint8 mc = m->GetClass();
                if (mc == CLASS_WARRIOR || mc == CLASS_PALADIN ||
                    mc == CLASS_DEATH_KNIGHT || mc == CLASS_DRUID)
                    if (m->GetMaxHealth() > player->GetMaxHealth() * 12 / 10)
                        tankish = true;
        ...
        else
        {
            ...
            if (Unit* sel = player->GetSelectedUnit())
                if (sel->IsAlive() && player->IsFriendlyTo(sel))
                    consider(sel, false);
        }
...
public:
    ...
    static bool HandleComboCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!tok.empty())
        {
            ...
            {
                st.on = false;
                handler->PSendSysMessage("|cffff8000[连招]|r 已关闭。共施放 %u 次。", st.castCount);
                return true;
            }
    ...
    static bool BuffNow(ChatHandler* handler, Player* player)
    {
        ...
        for (CombatSpec::Skill const& sk : plan.opener)
        {
            ...
            if (!real)
                continue;
            // v3.2：按整条 rank 链判断，低阶版本也算"已经有了"
            if (CombatHelper::HasAnyRankAura(player, sk.spell))
                continue;
            // v3.2：之前失败过（比如缺材料）的，别再排进队列
            if (CombatHelper::IsBackedOff(player, real))
                continue;
        ...
        if (queue.empty())
        {
            handler->PSendSysMessage("|cff00ff00[增益]|r 该上的都在身上了，不用补。");
            return true;
        }

        ObjectGuid guid = player->GetGUID();
        uint32 delay = 0;
        for (uint32 spellId : queue)
        {
            player->m_Events.AddEventAtOffset([guid, spellId]()
            {
                ...
                /*
                 * v3.2 修复：这条队列之前【绕过了退避表，也不看返回值】。
                 *
                 * 用户实测发现「关掉 .buff 自动补增益，.combo 就正常」——
                 * 原因就在这：v3.1 只修了 ComboTick 里的 tryCast，
                 * 但 .buff 排的队是独立事件，照样每次都硬放缺材料的祝福。
                 *
                 * 现在这里也走同一套退避表，两条路径行为一致。
                 */
                if (CombatHelper::IsBackedOff(p, spellId))
                    return;

                SpellCastResult res = p->CastSpell(p, spellId, CastSpellExtraArgs(false));
                if (res != SPELL_CAST_OK)
                    CombatHelper::MarkFailed(p, spellId, res);
                else
                    CombatHelper::ClearFailed(p, spellId);
            }, Milliseconds(delay));
                            ...
                            CombatSpec::SceneTuning const& tune)
    {
        ...
        // --- AOE：周围至少 2 个才值得放 ---
        if ((sk.flags & CombatSpec::SF_AOE) && nearbyCount < tune.aoeThreshold)
            return 2;

        // --- DOT/debuff：目标身上已经有了就不重复上 ---
        if ((sk.flags & CombatSpec::SF_DEBUFF_KEEP) && target)
            if (target->HasAura(realSpell, player->GetGUID()))
                return 2;
        ...
        if (sk.flags & CombatSpec::SF_BUFF_KEEP)
        {
             ...
             *   0    = 真没有 -> 才去上
             */
            int32 remain = CombatHelper::AuraRemainMs(player, sk.spell);
            if (remain != 0)
            {
                if (remain < 0)               // 永久光环
                    return 2;
                if (remain > 300000)          // 还剩 5 分钟以上
                    return 2;
            }
        }
                                ...
                                CombatSpec::SceneTuning const& tune)
    {
        ...
        {
            if (ht.unit->HasAura(realSpell, player->GetGUID()))
                return 2;
            // HOT 可以提前上，血线放宽 10%
    ...
    static void ComboTick(Player* player)
    {
        ...
        static std::unordered_map<uint32, PlanCache> s_planCache;

        PlanCache& pc = s_planCache[player->GetGUID().GetCounter()];
        ...
        auto tryCast = [&](CombatSpec::Skill const& sk, Unit* castTarget) -> bool
        {
            ...
            // 之前失败过且还在冷静期 -> 直接跳过，别再撞墙
            if (CombatHelper::IsBackedOff(player, real))
                return false;

            uint8 r = CheckSkill(player, castTarget, sk, real, nearby, tune);
            if (r == 1) { blockedByMove = true; return false; }
            if (r != 0) return false;

            /*
             * 关键修复（v3.1）：CastSpell 返回 SpellCastResult（Object.h:532），
             * 之前【没有检查返回值】—— 施法失败也当成功，于是：
             *   圣骑士强效力量祝福缺【王者símbolo】材料 -> 每次都 SPELL_FAILED_REAGENTS
             *   -> 但 HasAura 永远为假 -> 下一跳又试 -> 无限刷屏
             *
             * 现在失败就记进退避表，短时间内不再重试。
             */
            ...
            if (res != SPELL_CAST_OK)
            {
                CombatHelper::MarkFailed(player, real, res);
                return false;           // 没放出去，让主循环继续往下找
            }
        ...
        for (CombatSpec::Skill const& sk : plan.emergency)
        {
            ...
            // 保命技只在血少时用；解控技由 CheckSkill 判断是否真被控
            if (!(sk.flags & CombatSpec::SF_FREE_SELF) &&
     ...
     * 又不会太频繁拖累性能。
     */
    static void ScheduleTick(Player* player)
    {
        ObjectGuid guid = player->GetGUID();
...
public:
    ...
    void OnLogout(Player* player) override
    {
        if (!player)
            return;
        auto it = CombatHelper::States().find(player->GetGUID().GetCounter());
        if (it != CombatHelper::States().end())
            it->second.on = false;
