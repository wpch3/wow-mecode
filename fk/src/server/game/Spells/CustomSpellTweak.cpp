...
{
    SpellInfo* si = sSpellMgr->_GetSpellInfo(spellId);
    if (!si)
    {
                                         ...
                                         char const* cls, char const* desc)
{
    ...
    if (!touched)
    {
        Log(spellId, cls, desc, false, "该法术不是召唤类");
        return false;
    }
...
void CustomSpellTweakMgr::ApplyWarrior()
{
    ...
    ScaleEffect(7266,  3.0f, C, "投掷(Rank2)伤害");
    ScaleEffect(7267,  3.0f, C, "投掷(Rank3)伤害");
...
void CustomSpellTweakMgr::ApplyPaladin()
{
    ...
    // 清洁术 —— 驱散
    ScaleCooldown(4987, 0.5f, C, "清洁术冷却");
}
...
void CustomSpellTweakMgr::ApplyDeathKnight()
{
    ...
    // 亡者大军 —— 8 只改 10 只
    SetSummonCount(42650, 10, C, "亡者大军数量");
...
void CustomSpellTweakMgr::ApplyWarlock()
{
    ...
    // 灵魂石 —— 团队复活
    ScaleCooldown(47883, 0.6f, C, "灵魂石冷却");
...
void CustomSpellTweakMgr::ApplyDruid()
{
    ...
    AddChainTargets(53251, 2, C, "野性成长目标数");
