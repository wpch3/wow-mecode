...
$ErrorActionPreference='Stop';$U=Join-Path $Workspace 'uploads';$R=Join-Path $U 'G17B1R4_ROLLBACK_BUILD_RESULT.txt';$E=New-Object Text.UTF8Encoding($false);New-Item -ItemType Directory $U -Force|Out-Null;[IO.File]::WriteAllText($R,'',$E)
...
try{
 W 'G17B1R4_ROLLBACK_BUILD_START';if(Get-Process worldserver -ErrorAction SilentlyContinue){throw 'stop worldserver first'}
 $t=Join-Path $SourceRoot 'src\server\scripts\Commands\cs_dragonriding.cpp';$tool=Join-Path $PSScriptRoot 'tools\apply_g17b1r4_source.py';$sol=Join-Path $BuildRoot 'TrinityCore.sln';$exe=Join-Path $BuildRoot 'bin\RelWithDebInfo\worldserver.exe';$pdb=Join-Path $BuildRoot 'bin\RelWithDebInfo\worldserver.pdb';foreach($f in @($t,$tool,$sol,$exe,$pdb)){if(-not(Test-Path $f -PathType Leaf)){throw "missing: $f"}}
 ...
 $pre='94ff80334783e8883f0811a1a7f76595d91b729cc43684f00273abb9d955628b';if((Get-FileHash $t -Algorithm SHA256).Hash.ToLowerInvariant()-cne $pre){throw 'B1R3 preimage not restored'}
 ...
 W 'G17B1R4_SOURCE_ROLLBACK=PASS';W 'G17B1R4_ROLLBACK_BUILD_RESULT=PASS';W ('RESULT_FILE='+$R);exit 0
}catch{W ('G17B1R4_ROLLBACK_ERROR='+$_.Exception.Message);W 'G17B1R4_ROLLBACK_BUILD_RESULT=FAIL';W 'STOP_DO_NOT_START_WORLDSERVER';exit 1}
