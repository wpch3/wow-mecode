...
$Result = Join-Path $UploadDir "G17C8_FIX_ROLLBACK_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$ServerDbc = Join-Path $BuildRoot "bin\RelWithDebInfo\dbc\Spell.dbc"
$StateFile = Join-Path $UploadDir "G17C8_FIX_STATE.txt"
...
try {
    ...
    $srvBackups = @(Get-ChildItem -LiteralPath $UploadDir -Directory -Filter "G17C8_Server_DBC_Backup_*" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    ...
    if (Test-Path -LiteralPath $StateFile -PathType Leaf) {
        ...
        if ($backupDir -and (Test-Path -LiteralPath $backupDir -PathType Container)) {
            ...
            $backupRoot = Join-Path $backupDir "before_G17C8_root.MPQ"
            $backupLocale = Join-Path $backupDir "before_G17C8_locale.MPQ"
    ...
    W "G17C8_ROLLBACK_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17C8_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17C8_ROLLBACK_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
