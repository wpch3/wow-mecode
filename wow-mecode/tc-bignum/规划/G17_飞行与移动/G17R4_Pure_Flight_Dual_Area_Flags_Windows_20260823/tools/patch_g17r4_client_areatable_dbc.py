#!/usr/bin/env python3
"""Patch both required client-only 3.3.5a old-world flight flags.
...
EXPECTED_OUTPUT_SHA256 = "1acef997a27f844a8abee9b477c44f2097f745168c6df85ece6b1a135568c233"
...
AREA_FLAG_FLYING = 0x00000400
AREA_FLAG_ENABLE_FLIGHT_BOUNDS_ON_MAP = 0x00004000
AREA_FLAG_CLIENT_FLY_MASK = AREA_FLAG_FLYING | AREA_FLAG_ENABLE_FLIGHT_BOUNDS_ON_MAP
...
def select_rows(info: Dbc) -> list[int]:
    ...
    for row in range(info.records):
        ...
        if combined & CITY_BLOCK:
            continue
        if flags & AREA_FLAG_CLIENT_FLY_MASK == AREA_FLAG_CLIENT_FLY_MASK:
            continue
        selected.append(row)
    return selected
...
def make_patched(info: Dbc, selected: list[int]) -> bytes:
    ...
    for row in selected:
        ...
        U32.pack_into(out, at, flags | AREA_FLAG_CLIENT_FLY_MASK)
    return bytes(out)
...
def verify_diff(original: Dbc, patched: bytes, selected: list[int]) -> None:
    ...
    for row in range(original.records):
        ...
        expected = before | AREA_FLAG_CLIENT_FLY_MASK if row in selected_set else before
        if after != expected:
...
def patch(input_path: Path, output_path: Path, report_path: Path | None) -> None:
    ...
    lines = [
        "G17R4_CLIENT_AREATABLE_PATCH=PASS",
        ...
        "PATCHED_MAPS=0,1",
        "CLIENT_FLIGHT_FLAGS_ADDED=0x00000400,0x00004000",
        "R3_SINGLE_FLAG_0x00000400_INCOMPLETE=True",
...
def verify(original_path: Path, patched_path: Path) -> None:
    ...
    print("G17R4_CLIENT_AREATABLE_VERIFY=PASS")
...
def main() -> int:
    ...
    try:
        ...
        print(f"G17R4_CLIENT_AREATABLE_ERROR={exc}", file=sys.stderr)
        return 1
