    ...
    Player const* player)
{
    if (!sConfigMgr->GetBoolDefault("WorldFlight.Enable", false) ||
        !sConfigMgr->GetBoolDefault("WorldFlight.AllowOldWorld", true) || !areaEntry || !player)
        return false;
    ...
    // G17-R3: old-world AreaTable rows do not reliably carry AREA_FLAG_INSIDE.
    // Check the live VMap-derived outdoor state before the client AreaTable
    // flyability gate is opened, so buildings remain server-authoritatively blocked.
    if (sConfigMgr->GetBoolDefault("WorldFlight.BlockIndoorAreas", true) &&
        ((areaFlags & AREA_FLAG_INSIDE) || !player->IsOutdoors()))
        return false;
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player /*= nullptr*/, bool strict /*= true*/) const
{
    ...
    if (HasAttribute(SPELL_ATTR4_CAST_ONLY_IN_OUTLAND))
    {
        ...
        bool const g17OldWorldAllowed = G17IsOldWorldFlightAllowed(map_id, zone_id, area_id, areaEntry, player);
        if (strict)
        {
            if (!areaEntry || !player)
                return SPELL_FAILED_INCORRECT_AREA;

            // G17-R2: the G17 old-world policy replaces both original flyable
            // predicates.  R1 still called CanFlyInZone unconditionally, so
            // direct pure-flight mount spells were rejected on maps 0/1 even
            // after the client submitted the cast.
            if (!g17OldWorldAllowed &&
                (!areaEntry->IsFlyable() || !player->CanFlyInZone(map_id, zone_id, this)))
                return SPELL_FAILED_INCORRECT_AREA;

            if (g17OldWorldAllowed)
                TC_LOG_INFO("spells",
                    "G17R2 old-world pure-flight location allowed: spell={} player={} map={} zone={} area={}",
                    Id, player->GetName(), map_id, zone_id, area_id);
        }
        else
        {
