...
$UploadDir=Join-Path $Workspace "uploads";$Result=Join-Path $UploadDir "G17B1R5_WINDOWS_BUILD_RESULT.txt";$Utf8NoBom=New-Object System.Text.UTF8Encoding($false)
$Target=Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp";$Tool=Join-Path $PSScriptRoot "tools\apply_g17b1r5_source.py"
...
$Pre="e9418704731a2d9cd5119cc2024079a2326802796d00bf24e88928dd17ea7059";$Post="35af002b09b5d8112bbc1aaa1750f4a6245adec8b7c91a7852d69bdd283668b8"
...
try{
 W "G17B1R5_WINDOWS_BUILD_START";W "SCOPE=WRAPPER_MOUNT_AND_NO_PARACHUTE_BLOCKED_EXIT";W "RUNS_SQL=False";W "MODIFIES_CLIENT=False";W "R5_CLIENT_STATE_MODIFIED=False"
 ...
 $before=(Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant();W "SOURCE_SHA256_BEFORE=$before";if($before-cne $Pre -and $before-cne $Post){throw "dragonriding source is not locked B1R4 or B1R5 image"};$first=($before-ceq $Pre)
 ...
 $com=$env:ComSpec;if(-not $com){throw "ComSpec missing"};$NativeSelfTestArgs=@('/d','/c','echo G17B1R5_NATIVE_STDOUT& echo G17B1R5_NATIVE_STDERR 1>&2& exit /b 0');$rc=Invoke-NativeLogged -FilePath $com -NativeArgs $NativeSelfTestArgs -Prefix 'NATIVE_SELFTEST';W "NATIVE_SELFTEST_EXIT=$rc";if($rc-ne 0){throw "native runner selftest failed"}
 ...
 $after=(Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant();W "SOURCE_SHA256_AFTER=$after";if($after-cne $Post){throw "B1R4 postimage SHA mismatch"};W "G17B1R5_SOURCE_APPLY_GATE=PASS"
 ...
 W "G17B1R5_WINDOWS_BUILD_RESULT=PASS";W "STOP_DO_NOT_RUN_SQL";W "NEXT=Start worldserver normally; verify wrapper mounts and no-parachute indoor cleanup";W "RESULT_FILE=$Result";exit 0
}catch{W ("G17B1R5_WINDOWS_BUILD_ERROR="+$_.Exception.Message);W "G17B1R5_WINDOWS_BUILD_RESULT=FAIL";W "STOP_DO_NOT_START_WORLDSERVER";W "RESULT_FILE=$Result";exit 1}
