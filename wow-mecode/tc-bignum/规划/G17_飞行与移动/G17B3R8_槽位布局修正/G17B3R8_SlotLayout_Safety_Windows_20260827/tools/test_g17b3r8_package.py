...
TOOL = PKG / "tools" / "apply_g17b3r8_source.py"
INSTALL_PS1 = PKG / "Install-Build-G17B3R8-Windows.ps1"
...
POST = "dcfa78dd92ac4491882b9e2ec5c18a8b0803d6fcbb01cbe553e7fc069ac0f487"
...
def t1_lineage():
    ok("T1 original == B3R7 preimage", sha(PKG / "original.cpp") == PRE_B3R7)
    ok("T1 payload == B3R7 postimage", sha(PKG / "payload.cpp") == POST)
    ok("T1 rollback == B3R6 (safety floor)", sha(PKG / "rollback_safe.cpp") == PRE_B3R6)
    for img, expect in [("original_src/src/server/scripts/Commands/cs_dragonriding.cpp", PRE_B3R7),
...
def t2_gate_simulation():
    ...
    ok("T2 tool PRE/POST/ROLLBACK constants", pre and post and rb
       and pre.group(1) == PRE_B3R7 and post.group(1) == POST and rb.group(1) == PRE_B3R6)
    ...
    ok("T2 13 upgradeable intermediates incl. B3R6+B3R7", len(inter) == 13 and PRE_B3R6 in inter and PRE_B3R7 in inter)
    ok("T2 PS1 fingerprint", "f2360d7e" in ps1 and "G17B3R8_WINDOWS_BUILD_RESULT" in ps1)
...
def t4_payload_content():
    ...
    ok("T4 page switch at slot 6 on BOTH pages (B3R8 safety)", payload.count("m_spells[5] = SPELL_PAGE_SWITCH;") == 2)
    ok("T4 brake at slot 7 on BOTH pages", payload.count("m_spells[6] = SPELL_GLIDE_BRAKE;") == 2)
    ok("T4 no B3R7 leftover ordering", "m_spells[5] = SPELL_GLIDE_BRAKE;" not in payload)
    ...
    ok("T4 diff has exactly 4 hunks (header 2 + layout 2)", len(hunks) == 4, f"hunks={len(hunks)}: {hunks}")
    added = [l for l in diff if l.startswith("+") and not l.startswith("+++")]
    removed = [l for l in diff if l.startswith("-") and not l.startswith("---")]
    # bounded diff: header note (5) + 2 page layouts (2+2) + helper (~28) + 2 call sites (2) = small
    ok("T4 bounded diff (added<=30, removed<=10)", len(added) <= 30 and len(removed) <= 10,
...
def main() -> int:
    print(f"G17B3R8_PACKAGE_SELFTEST")
    t1_lineage()
    t2_gate_simulation()
    t3_functional_check()
    t4_payload_content()
    t5_static()
    t6_sums()
    print(f"G17B3R8_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
