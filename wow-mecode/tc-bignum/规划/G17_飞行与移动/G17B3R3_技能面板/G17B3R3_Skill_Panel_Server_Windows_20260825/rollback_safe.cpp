...
void ExecuteCombatSkill(Creature* dragon, Spell* spell)
{
    ...
    // Cooldown on the vehicle -> visible on the action bar, kept across pages.
...
class spell_g17_combat_skill : public SpellScript
{
    ...
    {
        Creature* dragon = GetCaster() ? GetCaster()->ToCreature() : nullptr;
        if (!dragon || !G17Dragonriding::IsDragon(dragon) || !dragon->IsAIEnabled())
        ...
        // vehicle spell history is updated after each successful cast and is
        // re-delivered with the action bar, surviving page switches).
    ...
    {
        Creature* dragon = GetCaster() ? GetCaster()->ToCreature() : nullptr;
        if (dragon && G17Dragonriding::IsDragon(dragon))
...
void AddSC_dragonriding_commandscript()
{
    ...
    TC_LOG_INFO("server.loading",
        ">> G17-B3R2 multi-page skillbar LOADED: movement page (ascend/dive/boost/dash/brake/land) <-> archetype combat page (5 skills), page-switch button, 8 slots, energy+cooldowns persist");
}
