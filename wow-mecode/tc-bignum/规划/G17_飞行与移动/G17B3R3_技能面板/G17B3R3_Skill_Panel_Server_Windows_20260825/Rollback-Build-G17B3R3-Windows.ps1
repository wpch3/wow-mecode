...
# G17-B3R3 rollback: source -> B3R2d floor (175e5a12), remove the
# G17DragonBar addon (restoring a backed-up copy if one exists), rebuild.
param(
...
$Result = Join-Path $UploadDir "G17B3R3_WINDOWS_ROLLBACK_RESULT.txt"
...
try {
    W "G17B3R3_WINDOWS_ROLLBACK_START"
    ...
    $backups = @(Get-ChildItem -LiteralPath $UploadDir -Directory -Filter "G17B3R3_Addon_Backup_*" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    ...
    W "G17B3R3_WINDOWS_ROLLBACK_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R3_WINDOWS_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17B3R3_WINDOWS_ROLLBACK_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
