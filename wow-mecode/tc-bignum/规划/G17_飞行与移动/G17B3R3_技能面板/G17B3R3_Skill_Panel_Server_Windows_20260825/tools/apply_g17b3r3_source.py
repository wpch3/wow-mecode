...
PRE_SHA256 = "175e5a122765691448738c7db7a25b32535f1fc29d7781e297e10614d4173975"   # B3R2d
POST_SHA256 = "29f3e55470f3ceaab79c8c5a6145ece76a8743c99999adec505a446239c32b3a" # B3R3 skill panel
...
INTERMEDIATE5_SHA256 = "a65b0ddcd06a66cfbdf04a91cd4114295615f9ee0c014f92bd742cb6c245b24d" # B3R2 r1c (if r1d not yet installed)
SAFE_ROLLBACK_SHA256 = "175e5a122765691448738c7db7a25b32535f1fc29d7781e297e10614d4173975"
...
def atomic_write(path: Path, data: bytes) -> None:
    temporary = path.with_name(path.name + ".g17b3r3.tmp")
    if temporary.exists():
...
def state_for_digest(digest: str) -> str:
    states = {PRE_SHA256: "READY_B3R3_PREIMAGE", POST_SHA256: "B3R3_APPLIED",
              SAFE_ROLLBACK_SHA256: "B3R3_SAFE_ROLLBACK"}
    for h in UPGRADEABLE_SHAS:
        states[h] = "B3R3_INTERMEDIATE_UPGRADEABLE"
...
def check(root: Path) -> str:
    ...
    print(f"G17B3R3_SOURCE_STATE={state}")
...
def apply(root: Path) -> None:
    state = check(root)
    target = root / SOURCE_RELATIVE
    if state == "B3R3_APPLIED":
        print("G17B3R3_SOURCE_APPLY=ALREADY_CURRENT")
        return
    backup = target.with_name(target.name + ".g17b3r3.preimage")
    if state in ("READY_B3R3_PREIMAGE", "B3R3_INTERMEDIATE_UPGRADEABLE") and not backup.exists():
    ...
    print("G17B3R3_SOURCE_APPLY=PASS")

def rollback(root: Path) -> None:
    state = check(root)
    target = root / SOURCE_RELATIVE
    if state == "B3R3_SAFE_ROLLBACK":
        print("G17B3R3_SOURCE_ROLLBACK=ALREADY_B3R2D_FLOOR")
        return
    ...
    print("G17B3R3_SOURCE_ROLLBACK=PASS_B3R2D_FLOOR")
