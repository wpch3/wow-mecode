...
// They are all valid 3.3.5a SpellVisualKit.dbc IDs and are layered so the
// boost actually reads on screen: launch burst + sustained ribbon trail +
// mid-boost energy pulse + top-speed wind burst + shutdown burst.  The
// mechanical jet flame is type-gated to mechanical/rocket archetypes only.
...
// B2R2 richer boost layers (all audited as no-aura/no-damage client kits):
constexpr uint32 VISUAL_KIT_BOOST_BURST       = 3783;  // Large generic blue speed burst on launch.
constexpr uint32 VISUAL_KIT_BOOST_GUST        = 713;   // Mid-boost air gust/whirl pulse.
constexpr uint32 VISUAL_KIT_BOOST_IMPACT      = 6482;  // Top-speed shockwave/impact ring.
constexpr uint32 VISUAL_KIT_DRAGON_DASH_FLASH = 9206;  // Dragon/beast dash flash (no fire model).
constexpr uint32 VISUAL_KIT_MAGIC_TRAIL       = 11818; // Magic/wind swirling caster trail (one-shot).
...
struct npc_g17_dragonriding_vehicle : public VehicleAI
{
    ...
    void DoAction(int32 action) override
    {
        ...
        if (action == ACTION_ACCELERATE)
        {
            ...
            // Layered launch feedback so the boost reads immediately.  All are
            // client-side one-shots (no aura, no damage, no state): a ChargeTrail
            // burst, a large speed burst and, for magic/wind archetypes, a
            // swirling caster trail; mechanical mounts also fire the jet flame.
            me->SendPlaySpellVisualKit(VISUAL_KIT_BOOST_LAUNCH, 0);
            me->SendPlaySpellVisualKit(VISUAL_KIT_BOOST_BURST, 0);
            if (_archetype == ARCHETYPE_MAGIC)
                me->SendPlaySpellVisualKit(VISUAL_KIT_MAGIC_TRAIL, 0);
            else if (_archetype == ARCHETYPE_DRAGON || _archetype == ARCHETYPE_BEAST)
                me->SendPlaySpellVisualKit(VISUAL_KIT_DRAGON_DASH_FLASH, 0);
            else if (_archetype == ARCHETYPE_MECHANICAL)
                me->SendPlaySpellVisualKit(VISUAL_KIT_MECHANICAL_THRUST, 0);
            if (_stalling)
                RecoverFromStall();
            SendToRider(me,
                "|cff80dfff[G17-B2R2] 高速推进启动：冲刺爆发、持续尾流与分段气浪已触发，正在平滑升档（硬上限1200%）。|r");
            return;
        }
...
private:
    ...
    void UpdateContinuousFlight(uint32 diff)
    {
        ...
        if (_boostTimer)
        {
            ...
            // A second, slower pulse layers an air gust over the ribbon trail so
            // the boost has a visible body instead of a single thin streamer.
            if (_boostGustTimer > diff)
                _boostGustTimer -= diff;
            else
            {
                _boostGustTimer = BOOST_GUST_INTERVAL_MS;
                me->SendPlaySpellVisualKit(VISUAL_KIT_BOOST_GUST, 0);
            }
        }
        else if (wasBoosting)
        {
            ...
            _boostGustTimer = 0;
            _boostTopSpeedAnnounced = false;
            // Shutdown feedback: a wind burst plus a visible impact ring so the
            // end of the boost reads on screen instead of silently cutting off.
            me->SendPlaySpellVisualKit(VISUAL_KIT_WIND_BURST, 0);
            me->SendPlaySpellVisualKit(VISUAL_KIT_BOOST_IMPACT, 0);
            SendToRider(me,
        ...
        if (_boostTimer && !_boostTopSpeedAnnounced && _currentSpeedRate >= 11.5f)
        {
            ...
            // Top-speed punch: wind burst + a visible shockwave/impact ring so
            // crossing into the top tier is unmistakable, not just a chat line.
            me->SendPlaySpellVisualKit(VISUAL_KIT_WIND_BURST, 0);
            me->SendPlaySpellVisualKit(VISUAL_KIT_BOOST_IMPACT, 0);
            SendToRider(me,
    ...
    void StartForwardClimb()
    {
        ...
        float exitHeading = me->GetOrientation();
    ...
    LandingProfile GetLandingProfile() const
    {
        ...
        switch (_archetype)
        {
            case ARCHETYPE_MAGIC: return { 26.0f, 7.0f, 12.0f, 14.0f, 8, true };
            case ARCHETYPE_DRAGON: return { 45.0f, 12.0f, 18.0f, 21.0f, 10, false };
            case ARCHETYPE_MECHANICAL: return { 32.0f, 9.0f, 16.0f, 19.0f, 9, false };
            case ARCHETYPE_BEAST: return { 18.0f, 6.0f, 10.0f, 16.0f, 7, false };
            default: return { 28.0f, 8.0f, 15.0f, 17.0f, 8, false };
        }
    }
