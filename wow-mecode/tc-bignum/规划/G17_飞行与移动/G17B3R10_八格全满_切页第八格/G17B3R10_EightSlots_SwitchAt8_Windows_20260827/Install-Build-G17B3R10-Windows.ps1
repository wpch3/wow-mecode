...
# (user directive after the C11 client-mod verification: the stock bar now
# renders 8 native buttons; B3R8 filled only 7).  Layout on BOTH pages:
#   [skills/moves 1-5] [制动@6] [生成器/俯冲@7] [切页@8]
# Plus instant chat feedback on every page switch and a mount hint, so a
# failed switch is visible immediately.  Includes all B3R9 dual-caster
# improvements (one install brings any lineage state fully up to date).
# Works from B3R6 / B3R7 / B3R8 / B3R9 source states. Payload 199cff4a, rollback 3fdb46e8.
param(
...
$Result = Join-Path $UploadDir "G17B3R10_WINDOWS_BUILD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Target = Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp"
$Tool = Join-Path $PSScriptRoot "tools\apply_g17b3r10_source.py"
...
try {
    W "G17B3R10_WINDOWS_BUILD_START"
    ...
    W "G17B3R10_SOURCE_APPLY_GATE=PASS"
    ...
    W "G17B3R10_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and look for 'G17-B3R6 performance fix LOADED'"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R10_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R10_WINDOWS_BUILD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
