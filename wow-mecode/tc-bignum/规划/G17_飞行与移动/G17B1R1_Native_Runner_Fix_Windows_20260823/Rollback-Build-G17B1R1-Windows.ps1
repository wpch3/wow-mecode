...
$ErrorActionPreference="Stop";$U=Join-Path $Workspace 'uploads';$R=Join-Path $U 'G17B1R1_ROLLBACK_BUILD_RESULT.txt';$E=New-Object Text.UTF8Encoding($false);New-Item -ItemType Directory $U -Force|Out-Null;[IO.File]::WriteAllText($R,'',$E)
...
function Invoke-NativeLogged {
 ...
 $old=$ErrorActionPreference;$o=@();$rc=9009
 try{$ErrorActionPreference='Continue';$o=@(& $FilePath @NativeArgs 2>&1);$rc=$LASTEXITCODE}finally{$ErrorActionPreference=$old}
 foreach($l in $o){W ($Prefix+'|'+$l)};return [int]$rc
}
try{
 W 'G17B1R1_ROLLBACK_BUILD_START';if(Get-Process worldserver -ErrorAction SilentlyContinue){throw 'stop worldserver first'}
 ...
 $RollbackArgs=@($tool,'rollback','--source-root',$SourceRoot);$rc=Invoke-NativeLogged -FilePath $py -NativeArgs $RollbackArgs -Prefix 'SOURCE_ROLLBACK';if($rc-ne 0){throw 'source rollback failed'}
 ...
 [IO.File]::SetLastWriteTimeUtc($t,[DateTime]::UtcNow);$before=(Get-Item $exe).LastWriteTimeUtc;Start-Sleep -Milliseconds 150;$RollbackBuildArgs=@($sol,'/m','/t:worldserver','/p:Configuration=RelWithDebInfo','/p:Platform=x64','/verbosity:minimal');$rc=Invoke-NativeLogged -FilePath $ms -NativeArgs $RollbackBuildArgs -Prefix 'MSBUILD';if($rc-ne 0){throw 'rollback MSBuild failed'};if((Get-Item $exe).LastWriteTimeUtc-le $before){throw 'rollback exe not updated'}
 W 'G17B1_SOURCE_ROLLBACK=PASS';W 'G17B1R1_ROLLBACK_BUILD_RESULT=PASS';W ('RESULT_FILE='+$R);exit 0
}catch{W ('G17B1R1_ROLLBACK_ERROR='+$_.Exception.Message);W 'G17B1R1_ROLLBACK_BUILD_RESULT=FAIL';W 'STOP_DO_NOT_START_WORLDSERVER';exit 1}
