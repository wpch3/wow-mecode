...
void WriteMovementPage(Creature* dragon)
{
    ...
    dragon->m_spells[5] = SPELL_GLIDE_BRAKE;  // B3-R7: 滑翔/制动 (dedicated 8-slot
                                              // G17DragonBar shows slots 6/7 that the
                                              // stock 6-button VehicleMenuBar hides)
    dragon->m_spells[6] = SPELL_PAGE_SWITCH;  // 切换技能页 (always last)
    dragon->m_spells[7] = 0;
}
...
void WriteCombatPage(Creature* dragon, uint32 mountArchetype)
{
    ...
    dragon->m_spells[5] = SPELL_GLIDE_BRAKE;  // B3-R7: 滑翔/制动 (escape tool on both pages)
    dragon->m_spells[6] = SPELL_PAGE_SWITCH;  // 切换技能页 (always last)
    dragon->m_spells[7] = 0;
}
