...
# G17-B3R7: seven-slot vehicle pages + vehicle cooldown UI packets.
#   1. both pages now fill m_spells[0..6] (brake 990028 back on the bar; the
#      page switch stays last). Stock client bar still shows 6; G17DragonBar
#      (G17-C10 addon) shows all bonus-bar slots including the 7th.
#   2. after every successful combat skill and the page switch, the rider's
#      client receives SMSG_SPELL_COOLDOWN (dragon GUID + player GUID
#      variants via SpellHistory::BuildCooldownPacket) so vehicle bar
#      buttons render the cooldown swirl (root cause: DBC RecoveryTime=0
#      removed local prediction; vehicle casts never hit the player-session
#      packet path in SpellHistory::StartCooldown).
# Source-only change, no DBC/SQL/addon steps. Payload f2360d7e, rollback 3fdb46e8.
param(
...
$Result = Join-Path $UploadDir "G17B3R7_WINDOWS_BUILD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Target = Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp"
$Tool = Join-Path $PSScriptRoot "tools\apply_g17b3r7_source.py"
...
try {
    ...
    W "G17B3R7_SOURCE_APPLY_GATE=PASS"
    ...
    W "G17B3R7_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and look for 'G17-B3R6 performance fix LOADED'"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R7_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R7_WINDOWS_BUILD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
