...
SOURCE_EXPECTED = {
    ...
    "payload_src/src/server/scripts/Commands/cs_dragonriding.cpp":
        "1febdecb17d0dbcb17aa831c7a2a4e589f4f3fb8f6855ab41faf8a13ae7bdcc4",
...
def main() -> int:
    ...
    for rel in ("tools/apply_g17b3r5_source.py", "tools/patch_g17b3r5_visuals.py",
                "tools/ps_static_check.py", "tests/test_g17b3r5.py",
                "addon_src/G17DragonBar/G17DragonBar.lua",
                "addon_src/G17DragonBar/G17DragonBar.toc",
                "Install-Build-G17B3R5-Windows.ps1",
                "Rollback-Build-G17B3R5-Windows.ps1"):
        if not (root / rel).is_file():
            print(f"PACKAGE_FILE_MISSING {rel}")
            rc = 1

    install = (root / "Install-Build-G17B3R5-Windows.ps1").read_text(encoding="utf-8")
    for token in ("G17B3R5_WINDOWS_BUILD_RESULT=PASS",
                  "G17B3R5_ADDON_INSTALL=PASS",
                  "G17B3R5_VISUAL_PATCH=PASS",
                  "apply_g17b3r5_source.py",
                  "patch_g17b3r5_visuals.py"):
    ...
    print("G17B3R5_PACKAGE_SELF_TEST=" + ("PASS" if rc == 0 else "FAIL"))
    return rc
