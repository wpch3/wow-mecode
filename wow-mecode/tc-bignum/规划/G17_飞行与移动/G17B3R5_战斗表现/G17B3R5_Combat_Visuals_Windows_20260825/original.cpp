...
// B3-R4: the mount fights alongside the rider (user request: "when I fight,
// the mount auto-casts skills too").
constexpr uint32 AUTOCOMBAT_INTERVAL_MS = 3500;
constexpr int32  AUTOCOMBAT_ENERGY_COST = 5;
constexpr uint32 AUTOCOMBAT_MIN_ENERGY  = 20;  // never ground the rider
...
void WriteMovementPage(Creature* dragon)
{
    ...
    dragon->m_spells[5] = SPELL_PAGE_SWITCH;  // 切换技能页
    dragon->m_spells[6] = 0;                  // beyond the 6-button client bar
    dragon->m_spells[7] = 0;
}
...
struct npc_g17_dragonriding_vehicle : public VehicleAI
{
    ...
    void PassengerBoarded(Unit* passenger, int8 seatId, bool apply) override
    {
        ...
        if (apply)
        {
            ...
            _energyRegenTimer = 0;
            _autoCombatTimer = 0;
            G17Dragonriding::WriteMovementPage(me);
            player->VehicleSpellInitialize();
            // B3-R3: also grant the DF-style skill panel (rider spellbook) so
            // the skills are castable from the player's own bars / addon bar.
            G17Dragonriding::GrantSkillPanel(player, GetData(G17Dragonriding::DATA_ARCHETYPE));
            TC_LOG_INFO("scripts.g17.dragonriding",
                "G17B3R2d movement page installed: player={}({}) archetype={} bar=6visible(8slots-cap)",
                player->GetName(), player->GetGUID().ToString(),
        ...
        G17Dragonriding::RevokeCombatSkills(passenger->ToPlayer());
        G17Dragonriding::NormalizeRiderAfterExit(passenger->ToPlayer());
        me->DespawnOrUnsummon(500ms);
    }

    // B3-R4: the mount fights alongside the rider.  While the rider is in
    // combat (and not in a BG/arena), the dragon periodically strikes the
    // rider's selected/victim target with a generator-tier assisted hit.
    // Damage, kill and threat credit go to the RIDER; the strike draws from
    // the shared energy pool but never below AUTOCOMBAT_MIN_ENERGY so the
    // rider is never grounded by the auto attacks.
    void UpdateAutoCombat(uint32 diff)
    {
        ...
        if (!rider || !rider->IsInCombat() || rider->InBattleground() || rider->InArena())
            return;
        if (me->GetPower(POWER_ENERGY) < G17Dragonriding::AUTOCOMBAT_MIN_ENERGY)
            return;
        ...
        float const mult = G17Dragonriding::CombatDamageMultiplier(_archetype) *
            (victim->IsPlayer() ? G17Dragonriding::COMBAT_PLAYER_FACTOR : 1.0f);
        uint32 const amount = uint32(float(
            (G17Dragonriding::COMBAT_BASE_DAMAGE[0] / 2u) +
            rider->GetLevel() * G17Dragonriding::COMBAT_DAMAGE_PER_LEVEL) * mult);
        Unit::DealDamage(rider, victim, amount, nullptr, DIRECT_DAMAGE,
            G17Dragonriding::CombatSchool(_archetype), nullptr, true);
        me->ModifyPower(POWER_ENERGY, -int32(G17Dragonriding::AUTOCOMBAT_ENERGY_COST));
        me->SendMeleeAttackStart(victim);
        me->SendMeleeAttackStop(victim);
        victim->SendPlaySpellVisualKit(G17Dragonriding::VISUAL_KIT_IMPACT_RING, 0);
        TC_LOG_DEBUG("scripts.g17.dragonriding",
            "G17B3R4 auto combat: rider={}({}) target={} amount={} energy={}",
            rider->GetName(), rider->GetGUID().ToString(), victim->GetGUID().ToString(),
            amount, me->GetPower(POWER_ENERGY));
    }
...
void AddSC_dragonriding_commandscript()
{
    ...
    TC_LOG_INFO("server.loading", " ");
    TC_LOG_INFO("server.loading", ">> G17-B2R3 dragonriding LOADED  build=20260824-r3 (skill4 castable + skill3 anti-reverse)");
    ...
    TC_LOG_INFO("server.loading",
        ">> G17-B3R4 combat experience LOADED: level-scaled damage, combat visuals, range-fixed carriers (30yd), mount assisted strikes while the rider fights; G17DragonBar addon v2");
}
