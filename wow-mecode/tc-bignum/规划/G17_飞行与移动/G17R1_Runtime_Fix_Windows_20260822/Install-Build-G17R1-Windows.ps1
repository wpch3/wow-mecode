...
$ExpectedPayload = "10a7002db4c173e441836870da01dd3009a7ba470369ca1a89bdb399ee9b2f45"
$Workspace = "C:\Users\Administrator\Downloads\workspace"
$UploadDir = Join-Path $Workspace "uploads"
$Result = Join-Path $UploadDir "G17R1_WINDOWS_FIX_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$SourceInstaller = Join-Path $PSScriptRoot "tools\apply_g17r1_source.py"
...
try {
    Write-Result "G17R1_WINDOWS_FIX_START"
    ...
    $PayloadHashBeforeApply = (Get-FileHash -LiteralPath $Payload -Algorithm SHA256).Hash.ToLowerInvariant()
    Write-Result "LOADER_SHA256=$LoaderHash"
    Write-Result "PAYLOAD_SHA256_BEFORE_APPLY=$PayloadHashBeforeApply"
    if ($LoaderHash -cne $ExpectedLoader) { throw "loader postimage SHA mismatch: $LoaderHash" }
    if (($PayloadHashBeforeApply -cne "c9535dca3390ece6735e6ff6b7418ed99ff206628b5e8febd7b78b05cba999bd") -and
        ($PayloadHashBeforeApply -cne $ExpectedPayload)) {
        throw "dragonriding source is neither locked G17-B0 nor G17-R1: $PayloadHashBeforeApply"
    }
    Write-Result "G17R1_PRE_APPLY_SOURCE_GATE=PASS"

    $ComSpec = $env:ComSpec
    if (-not $ComSpec -or -not (Test-Path -LiteralPath $ComSpec -PathType Leaf)) {
        throw "ComSpec command processor not found"
    }
    $NativeSelfTestArgs = @("/d", "/c", "echo G17R1_NATIVE_STDOUT& echo G17R1_NATIVE_STDERR 1>&2& exit /b 0")
    ...
    Write-Result "G17R1_NATIVE_RUNNER_SELFTEST=PASS"

    if (-not (Test-Path -LiteralPath $SourceInstaller -PathType Leaf)) {
        throw "G17-R1 source installer missing: $SourceInstaller"
    }
    $PythonCandidates = @(
    ...
    if ($ApplyExit -ne 0) { throw "G17-R1 independent source apply failed; exit code=$ApplyExit" }

    $PayloadHashAfterApply = (Get-FileHash -LiteralPath $Payload -Algorithm SHA256).Hash.ToLowerInvariant()
    Write-Result "PAYLOAD_SHA256_AFTER_APPLY=$PayloadHashAfterApply"
    if ($PayloadHashAfterApply -cne $ExpectedPayload) {
        throw "dragonriding G17-R1 postimage SHA mismatch after apply: $PayloadHashAfterApply"
    }
    Write-Result "G17R1_SOURCE_APPLY_GATE=PASS"
    ...
    Write-Result "G17R1_CMAKE_RECONFIGURE_START"
    ...
    Write-Result "G17R1_CMAKE_SOURCE_MEMBERSHIP=PASS"

    $BuildStartUtc = [DateTime]::UtcNow
    Write-Result "G17R1_MSBUILD_START_UTC=$($BuildStartUtc.ToString('o'))"
    ...
    $C4018Hits = @(Select-String -LiteralPath $Result -Pattern "cs_dragonriding.cpp.*C4018" -CaseSensitive:$false)
    Write-Result "DRAGONRIDING_C4018_HITS=$($C4018Hits.Count)"
    if ($C4018Hits.Count -ne 0) { throw "tracked cs_dragonriding.cpp C4018 warning remains" }
    Write-Result "G17R1_C4018_FIX_GATE=PASS"
    Write-Result "G17R1_BUILD_SOURCE_GATE=PASS"

    Write-Result "G17R1_WINDOWS_BUILD_PASS=True"
    Write-Result "G17R1_WINDOWS_BUILD_RESULT=PASS"
    Write-Result "G17R1_WINDOWS_BUILD_COMPLETE"
    Write-Result "STOP_DO_NOT_START_WORLDSERVER"
    Write-Result "RESULT_FILE=$Result"
    exit 0
}
catch {
    Write-Result ("G17R1_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    Write-Result "G17R1_WINDOWS_BUILD_PASS=False"
    Write-Result "G17R1_WINDOWS_BUILD_RESULT=FAIL"
    Write-Result "STOP_DO_NOT_START_WORLDSERVER"
    Write-Result "RESULT_FILE=$Result"
    exit 1
}
