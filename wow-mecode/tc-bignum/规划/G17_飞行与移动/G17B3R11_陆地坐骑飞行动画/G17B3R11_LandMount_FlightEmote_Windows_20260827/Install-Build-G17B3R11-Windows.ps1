...
$Result = Join-Path $UploadDir "G17B3R11_WINDOWS_BUILD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Target = Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp"
$Tool = Join-Path $PSScriptRoot "tools\apply_g17b3r11_source.py"
...
try {
    W "G17B3R11_WINDOWS_BUILD_START"
    ...
    W "G17B3R11_SOURCE_APPLY_GATE=PASS"
    ...
    W "G17B3R11_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and look for 'G17-B3R6 performance fix LOADED'"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R11_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R11_WINDOWS_BUILD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
