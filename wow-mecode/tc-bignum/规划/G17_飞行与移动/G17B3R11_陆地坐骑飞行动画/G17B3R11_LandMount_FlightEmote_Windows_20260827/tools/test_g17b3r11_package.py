...
TOOL = PKG / "tools" / "apply_g17b3r11_source.py"
INSTALL_PS1 = PKG / "Install-Build-G17B3R11-Windows.ps1"
...
def t4_payload_content():
    ...
    ok("T4 bounded diff (added<=150, removed<=45)", len(added) <= 190 and len(removed) <= 50,
...
def main() -> int:
    print(f"G17B3R11_PACKAGE_SELFTEST")
    t1_lineage()
    t2_gate_simulation()
    t3_functional_check()
    t4_payload_content()
    t5_static()
    t6_sums()
    print(f"G17B3R11_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
