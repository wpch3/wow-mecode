...
PATCHER = PKG / "tools" / "patch_g17c14.py"
INSTALL_PS1 = PKG / "Install-G17C14-ClientDBC.ps1"
...
def t2_visual_table():
    ...
    ok("T2 level-up visual 222 BANNED", "222" not in re.search(r"COMBAT_VISUALS = \{(.*?)\}", src, re.S).group(1))
    ok("T2 990029 renamed to 裂地重击",
       "裂地重击" in src and src.count("突袭·俯冲打击") <= 2)  # old name allowed ONLY in the rename pass comments
    # every combat visual must be a known impact kit
    impact = {"7860","143","4302","4961","2253","3860","36","57","7732","3057","67","9152","784","173","107","7479","46","64","3077"}
...
def t3_functional():
    ...
    ok("T3 chain state -> FRESH", "G17C14_STATE=FRESH" in r.stdout, r.stdout)
    ...
    ok("T3 patch appends 2", "G17C14_APPENDED_RECORDS=2" in r.stdout and "G17C14_PATCH=PATCHED" in r.stdout, r.stdout)
    ...
    ok("T3 patched verifies COMPLETE", "G17C14_STATE=COMPLETE" in r.stdout, r.stdout)
...
def t4_gate_simulation():
    ...
    ok("T4 patcher verify works without --input", rv.returncode == 0 and "G17C14_PAYLOAD_VERIFY=PASS" in rv.stdout, rv.stdout + rv.stderr)
...
def t5_static():
    r = subprocess.run([sys.executable, str(PKG / "tools" / "ps_static_check.py"),
                        str(INSTALL_PS1), str(PKG / "Rollback-G17C14-ClientDBC.ps1")],
...
def t0_cmd_ps1_consistency():
    ...
    for cmd_name in ("01_Install_G17C14.cmd", "02_Rollback_G17C14.cmd"):
...
def main() -> int:
    print(f"G17C14_PACKAGE_SELFTEST")
    ...
    t5_static()
    t6_sums()
    print(f"G17C14_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
