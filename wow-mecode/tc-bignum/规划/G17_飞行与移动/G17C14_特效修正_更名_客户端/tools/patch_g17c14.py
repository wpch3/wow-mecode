...
Visual table (all impact-rendering; source spell in parentheses):
  ...
  FLIGHT:  990025 切页=2276(Dash) 990026 拉升=9959(Rocket Jump)
           ...
           990029 裂地重击=2355(War Stomp) 990030 御风=3719(Aspect of the Cheetah)
"""
...
G17C14_VERSION = "v1_visual_truth_rename"
FIELDS = 234
RECSIZE = FIELDS * 4
...
# C14: slot 2 = the SELF-HEAL slot -> TargetA=CASTER + Range=1 (visual plays
# on the dragon) + reverse-verified heal/shield visuals.  222 is BANNED: it is
# the "Learning" (483) level-up golden burst the user literally saw.
COMBAT_VISUALS = {
    0: [7860, 143, 4302, 4961, 2253],  # Dragon: 龙息✓ 火冲 冰川护体(自愈) 振翼✓ 炎爆
    1: [3860, 36, 57, 7732, 3057],     # Beast: 愤怒 闪电链 强效治疗(自愈) 暗影 fury 心爆
    2: [67, 9152, 784, 173, 107],      # Magic: 火球 死亡缠绕 真言盾(自愈) 闪电箭 冰新星
    3: [143, 7479, 46, 7732, 107],     # Mech: 火冲 流星 献祭之火(维修) 暗影 fury 冰新星
    4: [64, 36, 3077, 7732, 2253],     # Generic: 暗影箭 闪电链 快速治疗(自愈) 暗影 fury 炎爆
}
FLIGHT_VISUALS = {990025: 2276, 990026: 9959, 990027: 2276, 990028: 63}
NEW_SPELLS = [
    # (id, name, icon, visual, range_index, target_a, desc)
    (990029, "裂地重击", 50, 2355, TARGET_RANGE, TGT_UNIT_ENEMY,
     "重击目标区域，对8码内的敌人造成物理伤害。"),
...
def scan(recs: bytes, count: int):
    ...
    for i in range(count):
        ...
        if ID_LO <= sid <= ID_HI:
            ...
            vis_ok = struct.unpack_from("<I", recs, i * RECSIZE + VISUAL_COL * 4)[0] == COMBAT_VISUALS[block][slot]
            tgt_ok = struct.unpack_from("<I", recs, i * RECSIZE + TGT_A_COL * 4)[0] == (
                TGT_UNIT_CASTER if slot == 2 else TGT_UNIT_ENEMY)
            if vis_ok and tgt_ok:
                combat_ok += 1
...
def state_of(found, combat_ok, flight_ok, new_ok):
    ...
    if found == 25:
        return "FRESH"          # C2-C13 chain: carriers exist; C14 upgrades
                                # visuals/slot-2 retarget/rename on top
    return "PARTIAL"
...
def patch(recs: bytes, strings: bytes, count: int, append_missing: bool):
    ...
    for i in range(count):
        ...
        if ID_LO <= sid <= ID_HI:
            ...
            # C14: slot 2 is the self-heal - aim the visual at the CASTER so
            # the heal/shield kit plays on the dragon; other slots at enemy.
            set_field(out_recs, i, TGT_A_COL,
                      TGT_UNIT_CASTER if slot == 2 else TGT_UNIT_ENEMY)
            set_field(out_recs, i, VISUAL_COL, COMBAT_VISUALS[block][slot])
            set_field(out_recs, i, RANGE_COL, 1 if slot == 2 else TARGET_RANGE)
    ...
    # C14 rename: repoint 990029's name/desc offsets to freshly appended
    # strings ("突袭·俯冲打击" -> "裂地重击"); old strings become orphans.
    renamed = 0
    rename_tail = b""
    name_off_990029 = len(strings) + len(new_names)
    rename_tail += "裂地重击".encode("utf-8") + b"\x00"
    desc_off_990029 = len(strings) + len(new_names) + len(rename_tail)
    rename_tail += "重击目标区域，对8码内的敌人造成物理伤害。".encode("utf-8") + b"\x00"
    total = len(out_recs) // RECSIZE
    for i in range(total):
        sid = struct.unpack_from("<I", out_recs, i * RECSIZE)[0]
        if sid == 990029:
            struct.pack_into("<I", out_recs, i * RECSIZE + NAME_COL * 4, name_off_990029)
            struct.pack_into("<I", out_recs, i * RECSIZE + DESC_COL * 4, desc_off_990029)
            renamed += 1

    return bytes(out_recs) + new_records, strings + new_names + rename_tail, patched, appended, renamed
...
def main() -> int:
    ...
    if args.version:
        print("G17C14_VERSION=" + G17C14_VERSION)
        return 0
    if args.command == "verify":
        ...
        print("G17C14_PAYLOAD_VERIFY=PASS")
        return 0
    ...
    print(f"G17C14_RECORDS={count}")
    print(f"G17C14_CARRIERS_FOUND={found}")
    print(f"G17C14_COMBAT_IMPACT_OK={combat_ok}")
    print(f"G17C14_FLIGHT_VISUAL_OK={flight_ok}")
    print(f"G17C14_NEW_SPELLS_PRESENT={new_ok}")
    print(f"G17C14_STATE={state}")
    print(f"G17C14_INPUT_SHA256={sha(data)}")

    if args.command == "check":
        print("G17C14_CHECK=PASS" if state != "PARTIAL" else "G17C14_CHECK=FAIL")
        return 0 if state != "PARTIAL" else 2

    if state == "COMPLETE":
        print("G17C14_PATCH=ALREADY_COMPLETE")
        print("G17C14_RESULT=PASS")
        return 0

    out_recs, out_strings, patched, appended, renamed = patch(recs, strings, count, append_missing=True)
    ...
    print(f"G17C14_PATCHED_RECORDS={patched}")
    print(f"G17C14_APPENDED_RECORDS={appended}")
    print(f"G17C14_RENAMED_RECORDS={renamed}")
    print(f"G17C14_OUTPUT_SHA256={sha(bytes(out))}")
    print("G17C14_PATCH=PATCHED")
    print("G17C14_RESULT=PASS")
    return 0
