...
# G17-C14: visual TRUTH pass + 990029 rename (chain-MPQ repack):
#   1. 222 BANNED everywhere - it is the Learning(483) level-up golden
#      burst the user literally saw; slot-2 self-heals now use reverse-
#      verified heal/shield kits (Ice Barrier/Greater Heal/PW:S/Flash Heal).
#   2. Slot-2 records re-targeted: ImplicitTargetA 18->1 (CASTER) +
#      RangeIndex 4->1 so the heal visual plays ON the dragon (it had
#      nowhere to render before - the self-heal has no enemy target).
#   3. 990029 renamed 突袭·俯冲打击 -> 裂地重击 (the 俯冲 wording read as
#      a flight skill inside the combat page; string-block repoint).
# The FrameXML files pass through unchanged.
...
$Result = Join-Path $UploadDir "G17C14_CLIENTMOD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Patcher = Join-Path $PSScriptRoot "tools\patch_g17c14.py"
...
$BuildFingerprint = "v1_visual_truth_rename"
$StateFiles = @(
    (Join-Path $UploadDir "G17C14_CLIENTMOD_STATE.txt"),
...
$ResultStateFile = Join-Path $UploadDir "G17C14_CLIENTMOD_STATE.txt"
...
try {
    W "G17C14_CLIENTMOD_START"
    W ("G17C14_BUILD=" + $BuildFingerprint)
    ...
    if ($patcherText -notmatch 'G17C14_VERSION\s*=\s*"v1_visual_truth_rename"') {
        throw ("OBSOLETE_PACKAGE: patcher is not v1_visual_truth_rename")
    }
    W "G17C14_PATCHER_VERSION_CHECK=PASS v1_visual_truth_rename"
    ...
    $WorkRoot = Join-Path $UploadDir ("G17C14_Client_Work_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_" + $PID)
    ...
    $dbcStateLine = @($dbcCheck | Where-Object { $_ -match '^G17C14_STATE=' } | Select-Object -First 1)[0]
    if (-not $dbcStateLine) { throw "patcher check printed no state line" }
    if ($dbcStateLine -match 'G17C14_STATE=COMPLETE') {
        W "G17C14_STATE=ALREADY_COMPLETE"
        W "G17C14_CLIENT_DBC=ALREADY_CURRENT"
        W "G17C14_CLIENT_DBC_RESULT=PASS"
        W "RESULT_FILE=$Result"
        exit 0
    }
    if ($dbcStateLine -notmatch 'G17C14_STATE=FRESH') {
    ...
    $verStateLine = @($verOut | Where-Object { $_ -match '^G17C14_STATE=' } | Select-Object -First 1)[0]
    if ($verExit -ne 0 -or -not $verStateLine -or $verStateLine -notmatch 'G17C14_STATE=COMPLETE') {
        throw "patched Spell.dbc failed content verification (expected G17C14_STATE=COMPLETE)"
    }
    ...
    $BackupDir = Join-Path $UploadDir ("G17C14_Client_Backup_" + $Stamp)
    if (Test-Path -LiteralPath $BackupDir) { throw "backup directory already exists" }
    New-Item -ItemType Directory -Path $BackupDir | Out-Null
    $BackupRoot = Join-Path $BackupDir "before_G17C14_root.MPQ"
    ...
    $BackupLocale = Join-Path $BackupDir "before_G17C14_locale.MPQ"
    ...
    # --- swap root ---
    $TemporaryTarget = $RootMpq + ".g17c14.new.tmp"
    $SwapOld = $RootMpq + ".g17c14.old.tmp"
    ...
    # --- mirror locale ---
    $LocaleTmp = $LocaleMpq + ".g17c14.new.tmp"
    $LocaleSwap = $LocaleMpq + ".g17c14.old.tmp"
    ...
    W "G17C14_CLIENTMOD=PASS"
    W "G17C14_CLIENTMOD_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    ...
    W ("G17C14_CLIENTMOD_ERROR=" + $Message)
    W "G17C14_CLIENTMOD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
