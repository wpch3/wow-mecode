...
TOOL = PKG / "tools" / "apply_g17b3r13_source.py"
INSTALL_PS1 = PKG / "Install-Build-G17B3R13-Windows.ps1"
...
POST = "c051fcdadeb8dadabd1f1d295c89ef798728d2c932da4792dcb0ad41d8a1d269"
PRE_B3R12R1A = "a6074cde399875ecc6741647bee57ca11a0f5785ab784fd3fe265aa0e1fd54ee"
...
def t1_lineage():
    ok("T1 original == B3R12r12a preimage (user current)", sha(PKG / "original.cpp") == PRE_B3R12R1A)
    ok("T1 payload == B3R7 postimage", sha(PKG / "payload.cpp") == POST)
    ok("T1 rollback == B3R6 (safety floor)", sha(PKG / "rollback_safe.cpp") == PRE_B3R6)
    for img, expect in [("original_src/src/server/scripts/Commands/cs_dragonriding.cpp", PRE_B3R12R1A),
...
def t2_gate_simulation():
    ...
    ok("T2 tool PRE/POST/ROLLBACK constants", pre and post and rb
       and pre.group(1) == PRE_B3R12R1A and post.group(1) == POST and rb.group(1) == PRE_B3R6)
    ...
    ok("T2 20 upgradeable intermediates incl. B3R6-R12r12a", len(inter) == 20 and PRE_B3R6 in inter and PRE_B3R7 in inter and PRE_B3R8 in inter and PRE_B3R9 in inter and PRE_B3R10 in inter and PRE_B3R11R1A in inter)
    ok("T2 PS1 fingerprint", "c051fcda" in ps1 and "G17B3R13_WINDOWS_BUILD_RESULT" in ps1)
...
def t4_payload_content():
    ...
    ok("T4 flight emote helper present (B3R11)",
       "void UpdateFlightEmote(bool airborne)" in payload
    ...
    ok("T4 r1a namespace fix (heritage)",
       "G17Dragonriding::ARCHETYPE_BEAST" in payload
       and payload.count("_archetype == G17Dragonriding::ARCHETYPE_BEAST") == 1)
    ok("T4 target pre-validation present (the hidden-cooldown fix)",
       payload.count("SPELL_FAILED_BAD_TARGETS") >= 1
       and "GetSpell()->IsTriggered() && slot != 2" in payload
       and payload.count("player->GetSelectedUnit()") >= 3)
    ...
    ok("T4 r12a AoE ctor fix (heritage)",
       "AnyUnfriendlyUnitInObjectRangeCheck check(target, player, G17Dragonriding::SWOOP_RADIUS)" in payload)
    ok("T4 R13 rename: 裂地重击 in chat, no 俯冲打击",
       "裂地重击：%u 点范围伤害" in payload and "突袭·俯冲打击：%u" not in payload)
    ok("T4 rest implemented",
       "ACTION_REST" in payload and "御龙休憩" in payload
       and "REST_REGEN_PCT" in payload and payload.count("rider->isMoving()") == 1
       and "GetExactDist2d(_restX, _restY)" in payload)
    ok("T4 archetype test override via SetData refreshes the page",
       "SetData(G17Dragonriding::DATA_ARCHETYPE, archetype)" in payload
       and "ApplySkillPage(_combatPage);" in payload)
    ok("T4 commands registered",
       '"rest"' in payload and '"archetype"' in payload and "HandleRest" in payload and "HandleArchetype" in payload)
    ...
    ok("T4 slot 7 page-pure",
       payload.count("m_spells[6] = SPELL_WIND_STANCE;") == 1
       and payload.count("m_spells[6] = SPELL_SWOOP_STRIKE;") == 1
       and "m_spells[6] = COMBAT_SPELL_BASE" not in payload
       and "m_spells[6] = SPELL_DIVE;" not in payload)
    ok("T4 both new spells registered",
       payload.count("RegisterSpellScript(spell_g17_swoop_strike);") == 1
       and payload.count("RegisterSpellScript(spell_g17_wind_stance);") == 1)
    ...
    ok("T4 bounded diff (added<=150, removed<=45)", len(added) <= 420 and len(removed) <= 70,
...
def t8_dbc_sql():
    appender = (PKG / "tools" / "append_g17b3r12_spells.py").read_text(encoding="utf-8")
    ok("T8 appender has both spells",
       "990029" in appender and "990030" in appender and "突袭·俯冲打击" in appender and "御风姿态" in appender)
    ok("T8 appender idempotent states", "ALREADY_APPENDED" in appender and "PARTIAL" in appender and "MISSING" in appender)
    sqlf = (PKG / "sql" / "G17B3R12_world_variety_binding.sql").read_text(encoding="utf-8")
    ok("T8 SQL binds both scripts", "spell_g17_swoop_strike" in sqlf and "spell_g17_wind_stance" in sqlf
       and "G17B3R12_WORLD_VARIETY_BINDING=PASS" in sqlf)
    r = subprocess.run([sys.executable, str(PKG / "tools" / "append_g17b3r12_spells.py"), "--version"],
                       capture_output=True, text=True)
...
def main() -> int:
    print(f"G17B3R13_PACKAGE_SELFTEST")
    t1_lineage()
    t2_gate_simulation()
    t3_functional_check()
    t4_payload_content()
    ...
    print(f"G17B3R13_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
