/*
 * G17-B3R13: swoop strike renamed 裂地重击 (the 俯冲 wording read as a
 * flight skill inside the combat page - user report) + .dragon rest
 * (御龙休憩, B4 step 1: 5%/s HP+mana while hovering, movement cancels) +
 * .dragon archetype 0-4 (live archetype override for the per-archetype
 * application testing the user demanded).
 *
 * G17-B3R12 heritage: no-target hidden-cooldown fix + skill variety #1/#2.
...
constexpr int32  ACTION_REST        = 9;   // B3-R13 御龙休憩 toggle (B4 step 1)
constexpr int32  ACTION_SET_ARCH    = 10;  // B3-R13 .dragon archetype test override
constexpr float  REST_REGEN_PCT     = 0.05f; // 5% max HP+mana per second while resting
constexpr float  REST_CANCEL_DIST   = 4.0f;  // movement beyond this cancels rest

// ---- B3-R12 skill variety #1/#2 (client carriers 990029/990030) ----
constexpr uint32 SPELL_SWOOP_STRIKE     = 990029; // 裂地重击 (renamed B3-R13; combat slot 7, first AoE)
...
struct npc_g17_dragonriding_vehicle : public VehicleAI
{
    ...
    void SetData(uint32 id, uint32 value) override
    {
        switch (id)
        {
            ...
            case G17Dragonriding::DATA_ARCHETYPE:
                _archetype = value;
                // B3-R13: .dragon archetype live override refreshes the
                // combat page immediately so the tester sees the new set.
                if (me->IsAIEnabled() && GetRider(me))
    ...
    void DoAction(int32 action) override
    {
        ...
        // ---- B3-R13 御龙休憩 (B4 step 1): fast regen while hovering ----
        if (action == G17Dragonriding::ACTION_REST)
        {
            _resting = !_resting;
            if (_resting)
            {
                _restX = me->GetPositionX();
                _restY = me->GetPositionY();
                _restZ = me->GetPositionZ();
                _restTimer = 0;
                if (Player* rider = GetRider(me))
                    if (WorldSession* session = rider->GetSession())
                        ChatHandler(session).PSendSysMessage(
                            "|cff80ff80[G17] 御龙休憩：悬停恢复中（每秒回复5%%生命与法力）。移动或下龙将结束休憩。|r");
            }
            else
                SendRestOffMessage();
            return;
...
private:
    ...
    void SendRestOffMessage()
    {
        if (Player* rider = GetRider(me))
            if (WorldSession* session = rider->GetSession())
                ChatHandler(session).PSendSysMessage("|cff80dfff[G17] 御龙休憩结束。|r");
    }
    ...
    void UpdateContinuousFlight(uint32 diff)
    {
        ...
        // ---- B3-R13 御龙休憩 regen + cancellation ----
        if (_resting)
        {
            _restTimer += diff;
            Player* rider = GetRider(me);
            if (!rider || !me->IsAlive())
            {
                _resting = false;
            }
            else
            {
                ...
                {
                    _restTimer = 0;
                    rider->ModifyHealth(int32(rider->GetMaxHealth() * G17Dragonriding::REST_REGEN_PCT));
                    rider->ModifyPower(POWER_MANA, int32(rider->GetMaxPower(POWER_MANA) * G17Dragonriding::REST_REGEN_PCT));
                }
                if (rider->isMoving() ||
                    me->GetExactDist2d(_restX, _restY) > G17Dragonriding::REST_CANCEL_DIST ||
                    std::abs(me->GetPositionZ() - _restZ) > G17Dragonriding::REST_CANCEL_DIST)
                {
                    _resting = false;
                    SendRestOffMessage();
                }
    ...
    bool _resting = false;    // B3-R13 御龙休憩
    uint32 _restTimer = 0;
    float _restX = 0.0f, _restY = 0.0f, _restZ = 0.0f;
...
// ---- B3-R12 skill variety #1: 裂地重击 (990029, combat slot 7; renamed in R13) ----
...
class spell_g17_swoop_strike : public SpellScript
{
    ...
    void HandleStrike(SpellEffIndex effectIndex)
    {
        ...
        if (WorldSession* session = player->GetSession())
            ChatHandler(session).PSendSysMessage(
                "|cff80dfff[G17] 裂地重击：%u 点范围伤害（%u 个目标）。|r", amount, count);
    }
...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        static ChatCommandTable dragonCommandTable =
        {
            ...
            { "rest",    HandleRest,    rbac::RBAC_PERM_COMMAND_HELP, Console::No },
            { "archetype", HandleArchetype, rbac::RBAC_PERM_COMMAND_HELP, Console::No },
    ...
    // B3-R13: find the player's active G17 dragon (summoned vehicle).
    static Creature* FindActiveDragon(Player* player)
    {
        if (!player)
            return nullptr;
        if (Creature* vehicle = player->GetVehicleCreatureBase())
            if (G17Dragonriding::IsDragon(vehicle))
                return vehicle;
        return nullptr;
    }
    ...
    static bool HandleRest(ChatHandler* handler)
    {
        ...
        if (!player)
            return false;
        Creature* dragon = FindActiveDragon(player);
        if (!dragon || !dragon->IsAIEnabled())
        {
            handler->SendSysMessage("|cffff8040[G17] 御龙休憩需要先骑上御龙术坐骑。|r");
            return true;
        }
        dragon->AI()->DoAction(G17Dragonriding::ACTION_REST);
        return true;
    }

    static bool HandleArchetype(ChatHandler* handler, uint32 archetype)
    {
        Player* player = handler->GetPlayer();
        if (!player)
            return false;
        ...
        if (!dragon || !dragon->IsAIEnabled())
        {
            handler->SendSysMessage("|cffff8040[G17] .dragon archetype 需要先骑上御龙术坐骑。|r");
            return true;
        }
        if (archetype > 4)
        {
            handler->SendSysMessage("|cffff8040[G17] 用法：.dragon archetype 0-4（0通用 1猛兽 2龙类 3魔法 4机械）。|r");
            return true;
        }
        dragon->AI()->SetData(G17Dragonriding::DATA_ARCHETYPE, archetype);
        if (WorldSession* session = player->GetSession())
            ChatHandler(session).PSendSysMessage("|cff80dfff[G17] 原型已切换为 %s（战斗技能页已刷新；特效按该族显示）。|r",
                G17Dragonriding::ArchetypeName(archetype));
        return true;
    }
    ...
    static bool HandleHelp(ChatHandler* handler)
    {
        ...
        handler->SendSysMessage(".dragon auto on|off；.dragon mount <spellId>；.dragon summon/dismiss/status；.dragon rest 御龙休憩；.dragon archetype 0-4 原型测试");
...
void AddSC_dragonriding_commandscript()
{
    ...
    TC_LOG_INFO("server.loading", " ");
    TC_LOG_INFO("server.loading", ">> G17-B3R13 dragonriding LOADED  build=20260827-r13 (renamed strike + rest + archetype test + swoop ctor fix)");
