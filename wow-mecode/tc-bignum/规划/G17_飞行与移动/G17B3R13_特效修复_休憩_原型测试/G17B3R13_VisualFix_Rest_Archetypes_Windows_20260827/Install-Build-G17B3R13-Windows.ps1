...
# B3-R13: (1) swoop strike renamed 裂地重击 in all server chat strings
# (the 俯冲 wording read as a flight skill inside the combat page);
# (2) .dragon rest - 御龙休憩 (B4 step 1): 5%/s HP+mana while hovering,
# movement/dismount cancels; (3) .dragon archetype 0-4 - live archetype
# override so the user can test every archetype's skill set + visuals on a
# single mount (the per-archetype application testing they demanded).
# Server DBC append (990029/990030) + world SQL are idempotent re-runs of
# the B3R12 steps (ALREADY_APPENDED / PASS).  Payload c051fcda,
# rollback 3fdb46e8.
param(
...
$Result = Join-Path $UploadDir "G17B3R13_WINDOWS_BUILD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Target = Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp"
$Tool = Join-Path $PSScriptRoot "tools\apply_g17b3r13_source.py"
$Appender = Join-Path $PSScriptRoot "tools\append_g17b3r13_spells.py"
$Sql = Join-Path $PSScriptRoot "sql\G17B3R13_world_variety_binding.sql"
...
function Invoke-WorldSql {
    ...
    W ("G17B3R13_WORLD_SQL_GATE=PASS " + $PassMarker)
}
...
try {
    W "G17B3R13_WINDOWS_BUILD_START"
    W ("G17B3R13_BUILD=" + $B3R12_BUILD)
    W "SCOPE=CHECKCAST_TARGET_PREVALIDATION+SWOOP_STRIKE_990029+WIND_STANCE_990030+SLOT7_PAGE_PURE"
    W "RUNS_SQL=True_G17B3R13_WORLD_VARIETY_BINDING"
    ...
    W "G17B3R13_SOURCE_APPLY_GATE=PASS"

    # --- B3-R12 server Spell.dbc append (990029/990030) ---
    $dbcOut = @(& $Python $Appender check --input $ServerDbc 2>&1)
    $dbcExit = $LASTEXITCODE
    $dbcStateLine = @($dbcOut | Where-Object { $_ -match '^G17B3R13_SPELL_DBC_STATE=' } | Select-Object -First 1)[0]
    ...
    if ($dbcStateLine -match 'G17B3R13_SPELL_DBC_STATE=ALREADY_APPENDED') {
        W "G17B3R13_SERVER_DBC_APPEND=ALREADY_APPENDED"
    } elseif ($dbcStateLine -match 'G17B3R13_SPELL_DBC_STATE=PARTIAL') {
        throw "server Spell.dbc has PARTIAL 990029/990030 presence; refusing"
    } elseif ($dbcStateLine -notmatch 'G17B3R13_SPELL_DBC_STATE=MISSING') {
        throw ("Unexpected server Spell.dbc state: " + $dbcStateLine)
    } else {
        $dbBackup = Join-Path $UploadDir ("G17B3R13_Server_DBC_Backup_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
        New-Item -ItemType Directory -Path $dbBackup -Force | Out-Null
        Copy-Item -LiteralPath $ServerDbc -Destination (Join-Path $dbBackup "Spell.dbc.before_g17b3r13")
        $dbcTmp = $ServerDbc + ".g17b3r13.new"
        ...
        W "G17B3R13_SERVER_DBC_APPEND=PASS"
    }

    # --- B3-R12 world SQL binding ---
    Invoke-WorldSql -SqlFile $Sql -PassMarker "G17B3R13_WORLD_VARIETY_BINDING=PASS"
    ...
    W "G17B3R13_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and look for 'G17-B3R6 performance fix LOADED'"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R13_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R13_WINDOWS_BUILD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
