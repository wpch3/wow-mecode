...
$Result = Join-Path $UploadDir "G17B2R1_WINDOWS_ROLLBACK_RESULT.txt"
...
try {
    W "G17B2R1_WINDOWS_ROLLBACK_START"
    W "SCOPE=SAFE_ROLLBACK_WITH_VISUAL_FREE_ACTION_AND_FIRE_FREE_BEAST"
    W "RUNS_SQL=True"
    W "DATABASE=world"
    ...
    if ($rc -ne 0) { throw "source safety rollback failed" }
    $after = (Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant()
    W "SOURCE_SHA256_AFTER=$after"
    if ($after -cne $SafeRollback) { throw "B2R1 safety-rollback SHA mismatch" }
    W "G17B2R1_SOURCE_ROLLBACK_GATE=PASS_SAFE_FLOOR"
    ...
    if ($first -and $afterExeHash -ceq $beforeExeHash) { throw "first B2R1 safety rollback build did not change worldserver.exe SHA" }
    W "G17B2R1_WINDOWS_ROLLBACK_RESULT=PASS_SAFE_FLOOR"
    W "NEXT=Start worldserver normally; safe rollback keeps the visual-free landing command and fire-free beast visual"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B2R1_WINDOWS_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17B2R1_WINDOWS_ROLLBACK_RESULT=FAIL"
