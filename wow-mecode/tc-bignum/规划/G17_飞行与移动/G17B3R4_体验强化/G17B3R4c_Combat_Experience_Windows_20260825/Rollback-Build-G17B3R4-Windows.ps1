...
$Result = Join-Path $UploadDir "G17B3R4_WINDOWS_ROLLBACK_RESULT.txt"
...
try {
    W "G17B3R4_WINDOWS_ROLLBACK_START"
    ...
    $rangeBackups = @(Get-ChildItem -LiteralPath $UploadDir -Directory -Filter "G17B3R4_Server_DBC_Backup_*" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    if ($rangeBackups.Count -gt 0) {
        $spell = Join-Path $rangeBackups[0].FullName "Spell.dbc.before_g17b3r4"
        if (Test-Path -LiteralPath $spell -PathType Leaf) {
            Copy-Item -LiteralPath $spell -Destination $ServerDbc -Force
            W "SERVER_DBC_RANGE_RESTORED=$spell"
        } else { W "SERVER_DBC_RANGE_BACKUP_FILE_MISSING (kept current DBC)" }
    } else { W "SERVER_DBC_RANGE_BACKUP_NONE (kept current DBC)" }
    ...
    $backups = @(Get-ChildItem -LiteralPath $UploadDir -Directory -Filter "G17B3R4_Addon_Backup_*" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    ...
    W "G17B3R4_WINDOWS_ROLLBACK_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R4_WINDOWS_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17B3R4_WINDOWS_ROLLBACK_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
