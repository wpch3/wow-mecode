...
PRE_SHA256 = "29f3e55470f3ceaab79c8c5a6145ece76a8743c99999adec505a446239c32b3a"   # B3R3
POST_SHA256 = "7cb417b3cec7c6d93002c35c96a17748583d412308ac019bf2830fd496afa936" # B3R4b combat experience (GetLevel fix)
...
SAFE_ROLLBACK_SHA256 = "29f3e55470f3ceaab79c8c5a6145ece76a8743c99999adec505a446239c32b3a"
...
def atomic_write(path: Path, data: bytes) -> None:
    temporary = path.with_name(path.name + ".g17b3r4.tmp")
    if temporary.exists():
...
def state_for_digest(digest: str) -> str:
    states = {PRE_SHA256: "READY_B3R4_PREIMAGE", POST_SHA256: "B3R4_APPLIED",
              SAFE_ROLLBACK_SHA256: "B3R4_SAFE_ROLLBACK"}
    for h in UPGRADEABLE_SHAS:
        states[h] = "B3R4_INTERMEDIATE_UPGRADEABLE"
...
def check(root: Path) -> str:
    ...
    print(f"G17B3R4_SOURCE_STATE={state}")
...
def apply(root: Path) -> None:
    state = check(root)
    target = root / SOURCE_RELATIVE
    if state == "B3R4_APPLIED":
        print("G17B3R4_SOURCE_APPLY=ALREADY_CURRENT")
        return
    backup = target.with_name(target.name + ".g17b3r4.preimage")
    if state in ("READY_B3R4_PREIMAGE", "B3R4_INTERMEDIATE_UPGRADEABLE") and not backup.exists():
    ...
    print("G17B3R4_SOURCE_APPLY=PASS")

def rollback(root: Path) -> None:
    state = check(root)
    target = root / SOURCE_RELATIVE
    if state == "B3R4_SAFE_ROLLBACK":
        print("G17B3R4_SOURCE_ROLLBACK=ALREADY_B3R3_FLOOR")
        return
    ...
    print("G17B3R4_SOURCE_ROLLBACK=PASS_B3R3_FLOOR")
