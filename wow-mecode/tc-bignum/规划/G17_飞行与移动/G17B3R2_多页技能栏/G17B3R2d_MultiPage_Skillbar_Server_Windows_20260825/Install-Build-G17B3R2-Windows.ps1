...
# G17-B3R2: multi-page vehicle skill bar install.
#   1) source apply (cs_dragonriding.cpp -> B3R2 postimage)
#   2) server dbc\Spell.dbc append (4 records 990025-990028)
#   3) world SQL bindings + creature_template bar alignment
#   4) MSBuild /t:worldserver (fresh obj + exe/pdb proof)
# What B3R2 changes (user-directed rework of the B3 skill UX):
#   - the skill bar lives on the VEHICLE (8 slots, Creature::m_spells), NOT in
#     the player spellbook; boarding installs the pure MOVEMENT page (no attack
#     skills mixed in: 拉升/俯冲/推进/冲刺/制动/着陆/切页);
#   - a page-switch button swaps movement <-> per-archetype COMBAT page
#     (5 skills + switch) and re-sends SMSG_PET_SPELLS live;
#   - slot0 of the combat page is an energy GENERATOR; 俯冲 restores energy;
#     passive regen +1/2s; combat page caps speed at 600%;
#   - combat skills are cast BY the vehicle with damage/heal/kill credit
#     attributed to the RIDER; cooldowns on the vehicle history survive page
#     switches; BG/arena still fully blocked.
param(
...
$Result = Join-Path $UploadDir "G17B3R2_WINDOWS_BUILD_RESULT.txt"
...
$Appender = Join-Path $PSScriptRoot "tools\append_g17b3r2_spells.py"
$Tests = Join-Path $PSScriptRoot "tests\test_g17b3r2.py"
$PackageTest = Join-Path $PSScriptRoot "Test-G17B3R2-Package.py"
$Sql = Join-Path $PSScriptRoot "sql\G17B3R2_world_skill_pages.sql"
...
function Invoke-WorldSql {
    ...
    W ("G17B3R2_WORLD_SQL_GATE=PASS " + $PassMarker)
}

$B3R2_BUILD = "r1d_sixslot_fastland"
try {
    W "G17B3R2_WINDOWS_BUILD_START"
    W ("B3R2_BUILD=" + $B3R2_BUILD)
    W "SCOPE=B3R2_MULTIPAGE_SKILLBAR_8SLOTS+PAGE_SWITCH+ENERGY_LOOP+RIDER_ATTRIBUTION+DBC_990025_990028"
    ...
    W "UNIT_TEST_EXIT=$rc"
    if ($rc -ne 0) { throw "B3R2 unit tests failed" }
    ...
    if ($after -cne $Post) { throw "B3R2 postimage SHA mismatch" }
    W "G17B3R2_SOURCE_APPLY_GATE=PASS"

    # --- server Spell.dbc append for 990025-990028 (semantic, idempotent;
    #     the B3-R1 appender already owns 990000-990024) ---
    $dbcOut = @(& $python $Appender check --input $ServerDbc 2>&1)
    $dbcExit = $LASTEXITCODE
    $dbcStateLine = @($dbcOut | Where-Object { $_ -match '^G17B3R2_SPELL_DBC_STATE=' } | Select-Object -First 1)[0]
    ...
    if ($dbcStateLine -match 'G17B3R2_SPELL_DBC_STATE=ALREADY_APPENDED') {
        W "G17B3R2_SERVER_DBC_APPEND=ALREADY_APPENDED"
    } elseif ($dbcStateLine -match 'G17B3R2_SPELL_DBC_STATE=PARTIAL') {
        throw "server Spell.dbc has PARTIAL 990025-990028 presence; refusing"
    } elseif ($dbcStateLine -notmatch 'G17B3R2_SPELL_DBC_STATE=MISSING') {
        throw ("Unexpected server Spell.dbc state: " + $dbcStateLine)
    } else {
        $dbBackup = Join-Path $UploadDir ("G17B3R2_Server_DBC_Backup_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
        New-Item -ItemType Directory -Path $dbBackup -Force | Out-Null
        Copy-Item -LiteralPath $ServerDbc -Destination (Join-Path $dbBackup "Spell.dbc.before_g17b3r2")
        $dbcTmp = $ServerDbc + ".g17b3r2.new"
        ...
        W "G17B3R2_SERVER_DBC_APPEND=PASS"
    }

    # --- world SQL binding + template bar alignment ---
    Invoke-WorldSql -SqlFile $Sql -PassMarker "G17B3R2_WORLD_SKILL_PAGES=PASS"
    ...
    if ($before -cne $Post -and $afterExeHash -ceq $beforeExeHash) { throw "first B3R2 build did not change worldserver.exe SHA" }
    W "G17B3R2_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and confirm 'G17-B3R2 multi-page skillbar LOADED' in worldserver.log"
    W "NEXT=Start worldserver, then in game: summon any mount -> the vehicle bar shows 7 movement buttons (no fire breath) -> the last button switches to the 5 archetype combat skills -> switch back any time."
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R2_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R2_WINDOWS_BUILD_RESULT=FAIL"
