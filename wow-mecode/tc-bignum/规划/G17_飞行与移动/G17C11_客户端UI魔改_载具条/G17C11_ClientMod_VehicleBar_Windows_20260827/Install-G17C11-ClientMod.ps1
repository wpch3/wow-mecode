...
# G17-C11: REAL CLIENT MOD - the stock vehicle action bar gets EIGHT native
# slots by patching the client's own FrameXML (loaded from the G17 patch-MPQ
# chain, the same proven mechanism as every G17 client DBC patch):
#   1. Interface\FrameXML\VehicleMenuBar.lua : VEHICLE_MAX_ACTIONBUTTONS 6 -> 8
#      (the client's keybind routing in ActionButton.lua follows this constant)
#   2. Interface\FrameXML\VehicleMenuBar.xml : VehicleMenuBarActionButton7/8 added
# The chain's Spell.dbc / AreaTable.dbc are passed through byte-identically.
param(
...
$Result = Join-Path $UploadDir "G17C11_CLIENTMOD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Patcher = Join-Path $PSScriptRoot "tools\patch_g17c11.py"
...
$ExpectedLuaHash = "0d572a7fbb7d69005b02f22932fbdc8f64c06f0d1d4ddc52a048300f30bb1a3c"
$ExpectedXmlHash = "31563ecf8787054408bab4049f212b2b77aa56c12dd264d60aa03b1494f1e628"
$BuildFingerprint = "v1_vehiclebar_8slots"
$StateFiles = @(
    (Join-Path $UploadDir "G17C11_CLIENTMOD_STATE.txt"),
...
$ResultStateFile = Join-Path $UploadDir "G17C11_CLIENTMOD_STATE.txt"
...
try {
    W "G17C11_CLIENTMOD_START"
    W ("G17C11_BUILD=" + $BuildFingerprint)
    ...
    if ($patcherText -notmatch 'G17C11_VERSION\s*=\s*"v1_vehiclebar_8slots"') {
        throw ("OBSOLETE_PACKAGE: patcher is not v1_vehiclebar_8slots")
    }
    W "G17C11_PATCHER_VERSION_CHECK=PASS v1_vehiclebar_8slots"
    ...
    $WorkRoot = Join-Path $UploadDir ("G17C11_Client_Work_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_" + $PID)
    ...
    if ($OldLua.State -eq "HIT" -and $OldLua.Hash -ceq $ExpectedLuaHash) {
        W "G17C11_STATE=ALREADY_COMPLETE"
        W "G17C11_CLIENTMOD=ALREADY_CURRENT"
        W "G17C11_CLIENTMOD_RESULT=PASS"
    ...
    $BackupDir = Join-Path $UploadDir ("G17C11_Client_Backup_" + $Stamp)
    ...
    # --- swap root ---
    $TemporaryTarget = $RootMpq + ".g17c11.new.tmp"
    $SwapOld = $RootMpq + ".g17c11.old.tmp"
    ...
    # --- mirror locale ---
    $LocaleTmp = $LocaleMpq + ".g17c11.new.tmp"
    $LocaleSwap = $LocaleMpq + ".g17c11.old.tmp"
    ...
    W ("G17C11_CLIENTMOD_ERROR=" + $Message)
    W "G17C11_CLIENTMOD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
