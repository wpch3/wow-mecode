...
# G17-B3R6: performance fix - remove LearnSpell/RemoveSpell from mount/dismount
# (the lag the user reported), add target validation in CheckCast (prevent
# cooldown without target). Source-only change, no DBC/SQL/addon steps.
param(
...
$Result = Join-Path $UploadDir "G17B3R6_WINDOWS_BUILD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Target = Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp"
$Tool = Join-Path $PSScriptRoot "tools\apply_g17b3r6_source.py"
...
try {
    ...
    W "G17B3R6_SOURCE_APPLY_GATE=PASS"
    ...
    W "G17B3R6_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and look for 'G17-B3R6 performance fix LOADED'"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R6_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R6_WINDOWS_BUILD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
