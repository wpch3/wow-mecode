...
def do_append(args) -> int:
    ...
    if getattr(args, "report", ""):
        ...
        rep.write_text(
            "G17B3_SPELL_DBC_STATE=APPENDED\n"
            "G17B3_SPELL_DBC_APPEND=PASS\n"
            f"G17B3_INPUT_SHA256={sha(data)}\n"
            f"G17B3_OUTPUT_SHA256={sha(out)}\n"
            f"G17B3_RECORDS_BEFORE={count}\n"
            f"G17B3_RECORDS_AFTER={new_count}\n",
            encoding="utf-8")
...
def main() -> int:
    ...
    args = ap.parse_args()
    if args.version:
        print("G17B3_DBC_APPENDER_VERSION=" + G17B3_DBC_APPENDER_VERSION)
        return 0
