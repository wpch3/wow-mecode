...
$Result = Join-Path $UploadDir "G17C2_CLIENT_MPQ_UNLOCK_ROLLBACK_RESULT.txt"
...
try {
    W "G17C2_CLIENT_MPQ_UNLOCK_ROLLBACK_START"
    ...
    if (-not (Test-Path -LiteralPath $StateFile -PathType Leaf)) { throw "G17C2 state file missing" }
    $state = @{}
    ...
    W "G17C2_CLIENT_MPQ_UNLOCK_ROLLBACK=PASS"
    W "G17C2_CLIENT_MPQ_UNLOCK_ROLLBACK_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17C2_CLIENT_MPQ_UNLOCK_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17C2_CLIENT_MPQ_UNLOCK_ROLLBACK_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
