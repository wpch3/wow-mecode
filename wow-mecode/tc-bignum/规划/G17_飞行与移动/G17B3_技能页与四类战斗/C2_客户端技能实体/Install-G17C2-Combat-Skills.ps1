...
$Result = Join-Path $UploadDir "G17C2_CLIENT_MPQ_UNLOCK_RESULT.txt"
$StateFile = Join-Path $UploadDir "G17C2_CLIENT_MPQ_UNLOCK_STATE.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Tool = if ($MpqCliOverride) { $MpqCliOverride } else { Join-Path $PSScriptRoot "third_party\mpqcli-windows-amd64.exe" }
$Patcher = Join-Path $PSScriptRoot "tools\append_g17b3_spells.py"
...
$ExpectedPatchedSpellHash = "760d3f274ab63fc780867a7193717eeca73b194632b2f69f43cf399faf65e2fe"
...
try {
    ...
    if ($haveR4State -and $haveR5State) {
        ...
        $WorkRoot = Join-Path $UploadDir ("G17C2_Client_Work_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_" + $PID)
    ...
    if (Test-Path -LiteralPath $StateFile -PathType Leaf) {
        ...
        if ($C1["INSTALL_STATUS"] -ceq "PASS" -and $C1["CLIENT_ROOT"] -ine $ClientRoot -and
            ...
            throw "G17C2 state exists but installed Spell.dbc is not the unlocked image"
        }
    }
    ...
    if ($OldSpell.Hash -ceq $ExpectedPatchedSpellHash) {
        # The 25 B3 combat skills are already present (a prior C2 run).
        W "G17C2_SPELL_DBC_STATE=ALREADY_APPENDED"
        W "G17C2_CLIENT_COMBAT_SKILLS=ALREADY_CURRENT"
        W "G17C2_CLIENT_COMBAT_SKILLS_RESULT=PASS"
        W "NOTE=client Spell.dbc already contains the 25 G17-B3 combat skills; no write performed"
        W "RESULT_FILE=$Result"
        exit 0
    }
    if ($OldSpell.Hash -cne $ExpectedSpellHash) {
        throw ("R4 owned Spell.dbc must be the C1 unlocked image 03bf11fd; found " + $OldSpell.Hash + " - run G17-C1 first if not")
    }
    ...
    # Pre-append semantic verification is the hash gate above
    ...
    $patchReport = Join-Path $WorkRoot "G17C2_SPELL_DBC_APPEND_REPORT.txt"
    $rc = Invoke-NativeLogged -FilePath $Python -NativeArgs @($Patcher, "append", "--input", $OldSpell.Path, "--output", $GeneratedSpell, "--report", $patchReport) -Prefix "SPELL_DBC_APPEND"
    ...
    $BackupDir = Join-Path $UploadDir ("G17C2_Client_Unlock_Backup_" + $Stamp)
    if (Test-Path -LiteralPath $BackupDir) { throw "backup directory already exists" }
    New-Item -ItemType Directory -Path $BackupDir | Out-Null
    $BackupRoot = Join-Path $BackupDir ("before_G17C2_" + (Split-Path -Leaf $RootMpq))
    ...
    if ($LocaleAbsent) {
        ...
        $BackupLocale = Join-Path $BackupDir ("before_G17C2_" + (Split-Path -Leaf $LocaleMpq))
    ...
    W "G17C2_CLIENT_MPQ_UNLOCK=PASS"
    W "G17C2_CLIENT_MPQ_UNLOCK_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    ...
    W ("G17C2_CLIENT_MPQ_UNLOCK_ERROR=" + $Message)
    W "G17C2_CLIENT_MPQ_UNLOCK_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
} finally {
