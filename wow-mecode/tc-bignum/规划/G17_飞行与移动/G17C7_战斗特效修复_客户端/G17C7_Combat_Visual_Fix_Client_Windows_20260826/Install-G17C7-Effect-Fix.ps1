...
$Result = Join-Path $UploadDir "G17C7_EFFECT_FIX_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Patcher = Join-Path $PSScriptRoot "tools\patch_g17c7_effects.py"
...
try {
    W "G17C7_EFFECT_FIX_START"
    W "C7_BUILD=v1_school_damage"
    ...
    $srvState = @($srvOut | Where-Object { $_ -match '^G17C7_EFFECT_STATE=' } | Select-Object -First 1)[0]
    ...
    if ($srvState -match 'G17C7_EFFECT_STATE=COMPLETE') {
        W "G17C7_SERVER_DBC=ALREADY_COMPLETE"
    } else {
        $srvBackup = Join-Path $UploadDir ("G17C7_Server_DBC_Backup_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
    ...
    $WorkRoot = Join-Path $UploadDir ("G17C7_Client_Work_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_" + $PID)
    ...
    $cliState = @($cliOut | Where-Object { $_ -match '^G17C7_EFFECT_STATE=' } | Select-Object -First 1)[0]
    ...
    if ($cliState -match 'G17C7_EFFECT_STATE=COMPLETE') {
        W "G17C7_CLIENT_MPQ=ALREADY_COMPLETE"
    } else {
        # Patch
        ...
        # Backup
        $BackupDir = Join-Path $UploadDir ("G17C7_Client_Backup_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
        New-Item -ItemType Directory -Path $BackupDir -Force | Out-Null
        Copy-Item -LiteralPath $RootMpq -Destination (Join-Path $BackupDir "before_G17C7_root.MPQ")
        Copy-Item -LiteralPath $LocaleMpq -Destination (Join-Path $BackupDir "before_G17C7_locale.MPQ")
        ...
        W "G17C7_CLIENT_MPQ=PASS"
    }

    W ""
    W "G17C7_EFFECT_FIX_RESULT=PASS"
    ...
    W ("G17C7_EFFECT_FIX_ERROR=" + $_.Exception.Message)
    W "G17C7_EFFECT_FIX_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
} finally {
