...
$BuildFingerprint = "v1_real_visuals_no_dbc_cd"
try {
    ...
    W "C6_PATCHER_VERSION_CHECK=PASS v1_real_visuals_no_dbc_cd"
    ...
    $BackupDir = Join-Path $UploadDir ("G17C9_Client_Visuals_Backup_" + $Stamp)
    if (Test-Path -LiteralPath $BackupDir) { throw "backup directory already exists" }
    New-Item -ItemType Directory -Path $BackupDir | Out-Null
    $BackupRoot = Join-Path $BackupDir ("before_G17C9_" + (Split-Path -Leaf $RootMpq))
    ...
    $BackupLocale = Join-Path $BackupDir ("before_G17C9_" + (Split-Path -Leaf $LocaleMpq))
    ...
    $StateFile = Join-Path $UploadDir "G17C9_CLIENT_VISUALS_STATE.txt"
