...
Usage: python tools/test_g17c10_package.py   (from the package root)
"""
...
PATCHER = PKG / "tools" / "patch_g17c10.py"
INSTALL_PS1 = PKG / "Install-G17C10-All-Archetypes.ps1"
ROLLBACK_PS1 = PKG / "Rollback-G17C10-All-Archetypes.ps1"
...
import patch_g17c10 as p9  # noqa: E402
...
def t1_patcher_functional():
    ...
    ok("T1 check FRESH exit0", rc == 0 and "G17C10_STATE=FRESH" in so, so)
    rc, so = run_patcher(["patch", "--input", str(src), "--output", str(out)])
    ok("T1 patch exit0", rc == 0 and "G17C10_PATCH=PATCHED" in so and "G17C10_PATCHED_RECORDS=25" in so, so)
    data = out.read_bytes()
    ...
    ok("T1 patched verifies COMPLETE", rc == 0 and "G17C10_STATE=COMPLETE" in so, so)
    rc, so = run_patcher(["patch", "--input", str(out), "--output", str(PKG / "tools" / "_t1_out2.dbc")])
    ok("T1 idempotent ALREADY_COMPLETE", rc == 0 and "G17C10_PATCH=ALREADY_COMPLETE" in so, so)
    ...
    ok("T1 patches from C8-like state", rc == 0 and "G17C10_PATCH=PATCHED" in so, so)
    rc, so = run_patcher(["check", "--input", str(c8out)])
    ok("T1 C8-like result COMPLETE", rc == 0 and "G17C10_STATE=COMPLETE" in so, so)
    # PARTIAL refusal
    ...
    ok("T1 partial check FAIL exit2", rc == 2 and "G17C10_STATE=PARTIAL" in so, so)
    rc, so = run_patcher(["patch", "--input", str(part), "--output", str(partout)])
    ok("T1 partial patch REFUSED exit2", rc == 2 and "G17C10_PATCH=REFUSED" in so, so)
...
def t2_installer_gate_simulation():
    ...
    ver = re.search(r'G17C10_VERSION\s*=\s*"([^"]+)"', patcher_text)
...
def t5_stale_tokens():
    ...
    # T8: addon checks
    addon = PKG / "addon" / "G17DragonBar"
    toc = (addon / "G17DragonBar.toc")
    lua = (addon / "G17DragonBar.lua")
    xml = (addon / "G17DragonBar.xml")
    ok("T8 addon files present", toc.exists() and lua.exists() and xml.exists())
    if lua.exists():
        lt = lua.read_text(encoding="utf-8")
        ok("T8 cooldown table matches server contract",
           "SLOT_CD = { 4, 6, 20, 10, 60 }" in lt and "PAGE_SWITCH_CD = 1" in lt)
        ok("T8 uses native bonus action template",
           '"BonusActionButtonTemplate"' in lt)
        ok("T8 CLEU spellcast tracking present",
           "COMBAT_LOG_EVENT_UNFILTERED" in lt and "SPELL_CAST_SUCCESS" in lt)
        ok("T8 quest-mount separation (power type 3 + max 100 gate)",
           "UnitPowerMax" in lt and "vehicle" in lt and "strictName" in lt)
        ok("T8 no protected calls in combat paths",
           "InCombatLockdown" in lt)
    ok("T8 readme documents addon install", "G17DragonBar" in (PKG / "README_FIRST.txt").read_text(encoding="utf-8"))


def t6_rollback_real():
    t = ROLLBACK_PS1.read_text(encoding="utf-8")
    ok("T6 rollback non-empty and restores", len(t) > 2000
       and "before_G17C10_root.MPQ" in t and "before_G17C10_locale.MPQ" in t
       and "G17C10_Client_Backup_" in t and "CLIENT_CACHE_REMOVED" in t
       and "G17C10_ROLLBACK_RESULT=PASS" in t)
...
def main() -> int:
    print(f"G17C10_PACKAGE_SELFTEST package={PKG.name}")
    ...
    t6_rollback_real()
    t7_sums_complete()
    print(f"G17C10_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
