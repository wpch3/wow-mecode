...
# backup recorded in G17C10_CLIENT_VISUALS_STATE.txt (or the newest
# G17C10_Client_Backup_* directory in uploads), then clear the client cache.
param(
...
$Result = Join-Path $UploadDir "G17C10_ROLLBACK_RESULT.txt"
$StateFile = Join-Path $UploadDir "G17C10_CLIENT_VISUALS_STATE.txt"
...
try {
    ...
    if (-not $BackupDir -or -not (Test-Path -LiteralPath $BackupDir -PathType Container)) {
        # fallback: newest G17C10_Client_Backup_* directory
        $candidates = @(Get-ChildItem -LiteralPath $UploadDir -Directory -Filter "G17C10_Client_Backup_*" -ErrorAction SilentlyContinue | Sort-Object Name -Descending)
    ...
    W "G17C10_ROLLBACK_RESULT=PASS"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17C10_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17C10_ROLLBACK_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
