...
# G17-C9 v3: REAL per-skill visuals (Wowhead-verified, per archetype x slot)
# + remove DBC RecoveryTime/CategoryRecoveryTime (phantom cooldown fix).
#
...
$Result = Join-Path $UploadDir "G17C9_CLIENT_VISUALS_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Patcher = Join-Path $PSScriptRoot "tools\patch_g17c9.py"
...
$BuildFingerprint = "v3_wowhead_visuals_no_dbc_cd"
...
$ResultStateFile = Join-Path $UploadDir "G17C9_CLIENT_VISUALS_STATE.txt"
...
try {
    W "G17C9_CLIENT_VISUALS_START"
    W ("G17C9_BUILD=" + $BuildFingerprint)
    ...
    W "DBC_PAYLOAD=same Wowhead-verified visuals as C9 v2; installer rebuilt (v3)"
    ...
    if ($patcherText -notmatch 'G17C9_VERSION\s*=\s*"v3_wowhead_visuals_no_dbc_cd"') {
        throw ("OBSOLETE_PACKAGE: patcher is not v3_wowhead_visuals_no_dbc_cd")
    }
    W "G17C9_PATCHER_VERSION_CHECK=PASS v3_wowhead_visuals_no_dbc_cd"
    ...
    $WorkRoot = Join-Path $UploadDir ("G17C9_Client_Work_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_" + $PID)
    ...
    $cliState = @($cliOut | Where-Object { $_ -match '^G17C9_STATE=' } | Select-Object -First 1)[0]
    if (-not $cliState) { throw "client check printed no state" }
    W ("CLI_STATE=" + $cliState.ToString().Trim())

    if ($cliState -match 'G17C9_STATE=COMPLETE') {
        W "G17C9_VISUAL_STATE=ALREADY_COMPLETE"
        W "G17C9_CLIENT_VISUALS=ALREADY_CURRENT"
        W "G17C9_CLIENT_VISUALS_RESULT=PASS"
        W "NOTE=client Spell.dbc already has the v3 visuals+ranges+zero cooldowns; no write performed"
        W "RESULT_FILE=$Result"
        exit 0
    }
    if ($cliState -match 'G17C9_STATE=PARTIAL') {
    ...
    $verState = @($verOut | Where-Object { $_ -match '^G17C9_STATE=' } | Select-Object -First 1)[0]
    if ($verExit -ne 0 -or -not $verState -or $verState -notmatch 'G17C9_STATE=COMPLETE') {
        throw "patched Spell.dbc failed content verification (expected G17C9_STATE=COMPLETE)"
    }
    ...
    $BackupDir = Join-Path $UploadDir ("G17C9_Client_Backup_" + $Stamp)
    if (Test-Path -LiteralPath $BackupDir) { throw "backup directory already exists" }
    New-Item -ItemType Directory -Path $BackupDir | Out-Null
    $BackupRoot = Join-Path $BackupDir "before_G17C9_root.MPQ"
    ...
    $BackupLocale = Join-Path $BackupDir "before_G17C9_locale.MPQ"
    ...
    # --- swap root ---
    $TemporaryTarget = $RootMpq + ".g17c9.new.tmp"
    $SwapOld = $RootMpq + ".g17c9.old.tmp"
    ...
    # --- mirror locale ---
    $LocaleTmp = $LocaleMpq + ".g17c9.new.tmp"
    $LocaleSwap = $LocaleMpq + ".g17c9.old.tmp"
    ...
    W ("G17C9_CLIENT_VISUALS_ERROR=" + $Message)
    W "G17C9_CLIENT_VISUALS_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
