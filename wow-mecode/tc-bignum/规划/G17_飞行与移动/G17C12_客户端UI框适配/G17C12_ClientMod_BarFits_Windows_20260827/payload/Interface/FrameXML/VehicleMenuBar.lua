...
VEHICLE_MAX_ACTIONBUTTONS = 8; -- G17 client mod: 8 vehicle action buttons
-- (was 6; ActionButton.lua's keybind routing follows this constant automatically)
...
function VehicleMenuBar_UpdateActionBars()
	...
	-- G17 client mod v2 (C12): eight 62px-wide buttons overflow the stock
	-- vehicle skin art, which was sized for six.  Scale the whole action
	-- button frame down so eight buttons fit the original six-button
	-- footprint (6*62 / 8*62 = 0.75).  Tunable here if it looks too small.
	if ( VEHICLE_MAX_ACTIONBUTTONS > 6 ) then
		VehicleMenuBarActionButtonFrame:SetScale(0.75);
	end
end
