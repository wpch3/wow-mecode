#!/usr/bin/env python3
"""G17-C12: CLIENT MOD v2 - make EIGHT buttons fit the stock skin art.

C11 gave the stock vehicle bar eight native slots; user report: the UI frame
is too small for eight buttons (sized for six).  C12 scales the whole
VehicleMenuBarActionButtonFrame to 0.75 so eight 62px buttons fit the original
six-button footprint (6*62/8*62).  Tunable constant inside
VehicleMenuBar_UpdateActionBars.
Baseline = the C11-patched lua already installed on the user machine:
...
G17C12_VERSION = "v1_8slot_bar_fits"
...
PAYLOAD_LUA_SHA256 = "071e688725661511d8471b6c5534c7abbadaf0538e0e0fb34750a75ecf848ef6"
PAYLOAD_XML_SHA256 = "31563ecf8787054408bab4049f212b2b77aa56c12dd264d60aa03b1494f1e628"
BASELINE_LUA_SHA256 = "0d572a7fbb7d69005b02f22932fbdc8f64c06f0d1d4ddc52a048300f30bb1a3c"  # C11 image (installed)
...
def verify_payload() -> None:
    ...
    if lua.count("VehicleMenuBarActionButtonFrame:SetScale(0.75);") != 1:
        raise RuntimeError("payload lua missing the C12 scale fix")


def check_lua(path: Path) -> int:
    """Report the client-side state of an extracted VehicleMenuBar.lua."""
    text = path.read_text(encoding="utf-8", errors="replace")
    if "VehicleMenuBarActionButtonFrame:SetScale(0.75);" in text:
        state = "COMPLETE"
    elif "VEHICLE_MAX_ACTIONBUTTONS = 8;" in text:
        state = "FRESH"  # C11 without the scale fix
    else:
        state = "PARTIAL"
    print(f"G17C12_STATE={state}")
    print("G17C12_CHECK=PASS" if state != "PARTIAL" else "G17C12_CHECK=FAIL")
...
def main() -> int:
    ...
    if args.version:
        print("G17C12_VERSION=" + G17C12_VERSION)
        return 0
    if args.command == "verify":
        verify_payload()
        print("G17C12_PAYLOAD_VERIFY=PASS")
        return 0
