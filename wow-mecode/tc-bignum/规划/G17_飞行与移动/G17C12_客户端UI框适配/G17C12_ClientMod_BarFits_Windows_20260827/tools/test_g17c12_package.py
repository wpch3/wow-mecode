...
PATCHER = PKG / "tools" / "patch_g17c12.py"
INSTALL_PS1 = PKG / "Install-G17C12-ClientMod.ps1"
...
LUA_SHA = "071e688725661511d8471b6c5534c7abbadaf0538e0e0fb34750a75ecf848ef6"
XML_SHA = "31563ecf8787054408bab4049f212b2b77aa56c12dd264d60aa03b1494f1e628"
BASE_LUA_SHA = "0d572a7fbb7d69005b02f22932fbdc8f64c06f0d1d4ddc52a048300f30bb1a3c"  # C11 image
BASE_XML_SHA = "31563ecf8787054408bab4049f212b2b77aa56c12dd264d60aa03b1494f1e628"
...
def t1_hashes():
    ...
    ok("T1 baseline = installed C11 image unchanged",
       sha(C11 / "payload/Interface/FrameXML/VehicleMenuBar.lua") == BASE_LUA_SHA
       and sha(C11 / "payload/Interface/FrameXML/VehicleMenuBar.xml") == BASE_XML_SHA)


def t2_content():
    lua = PAYLOAD_LUA.read_text(encoding="utf-8")
    xml = PAYLOAD_XML.read_text(encoding="utf-8")
    ok("T2 constant = 8 kept from C11", lua.count("VEHICLE_MAX_ACTIONBUTTONS = 8;") == 1)
    ok("T2 scale fix present exactly once", lua.count("VehicleMenuBarActionButtonFrame:SetScale(0.75);") == 1)
    ok("T2 scale guarded to >6 buttons", "if ( VEHICLE_MAX_ACTIONBUTTONS > 6 ) then" in lua)
...
def t3_diff_contract():
    lua_old = Path("/home/user/wow-mecode/wow-mecode/tc-bignum/规划/G17_飞行与移动/G17C11_客户端UI魔改_载具条/G17C11_ClientMod_VehicleBar_Windows_20260827/payload/Interface/FrameXML/VehicleMenuBar.lua").read_text(encoding="utf-8")
    ...
    ok("T3 lua diff vs C11 is pure addition (7 lines: comment + guard + scale)", len(ld) == 7
       and all(l.startswith("+") for l in ld), str(ld))

    xml_old = Path("/home/user/wow-mecode/wow-mecode/tc-bignum/规划/G17_飞行与移动/G17C11_客户端UI魔改_载具条/G17C11_ClientMod_VehicleBar_Windows_20260827/payload/Interface/FrameXML/VehicleMenuBar.xml").read_text(encoding="utf-8")
    ...
    xd = [l for l in difflib.unified_diff(xml_old.splitlines(), xml_new.splitlines(), lineterm="", n=0)
          if (l.startswith("+") and not l.startswith("+++")) or (l.startswith("-") and not l.startswith("---"))]
    ok("T3 xml unchanged from C11", len(xd) == 0, f"len={len(xd)}")
...
def t5_gate_simulation():
    ...
    ok("T5 patcher verify passes", r.returncode == 0 and "G17C12_PAYLOAD_VERIFY=PASS" in r.stdout, r.stdout)


def t6_static():
    r = subprocess.run([sys.executable, str(PKG / "tools" / "ps_static_check.py"),
                        str(INSTALL_PS1), str(PKG / "Rollback-G17C12-ClientMod.ps1")],
...
def main() -> int:
    print(f"G17C12_PACKAGE_SELFTEST package={PKG.name}")
    t1_hashes()
    t2_content()
    t3_diff_contract()
    t4_xml()
    ...
    t6_static()
    t7_sums()
    print(f"G17C12_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
