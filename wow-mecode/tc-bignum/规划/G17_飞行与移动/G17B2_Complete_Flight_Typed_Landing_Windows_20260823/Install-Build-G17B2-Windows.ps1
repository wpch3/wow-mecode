...
$UploadDir=Join-Path $Workspace "uploads";$Result=Join-Path $UploadDir "G17B2_WINDOWS_BUILD_RESULT.txt";$Utf8NoBom=New-Object System.Text.UTF8Encoding($false)
$Target=Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp";$Tool=Join-Path $PSScriptRoot "tools\apply_g17b2_source.py"
...
$Pre="35af002b09b5d8112bbc1aaa1750f4a6245adec8b7c91a7852d69bdd283668b8";$Post="8b47a5b507bc281198363972e10f91ab0ed3784ad920cf810bd20eacfb6ec1d5"
...
try{
 W "G17B2_WINDOWS_BUILD_START";W "SCOPE=COMPLETE_MOMENTUM_FLIGHT_CLIMB_AND_TYPED_NO_PARACHUTE_LANDING";W "RUNS_SQL=False";W "MODIFIES_CLIENT=False";W "R5_CLIENT_STATE_MODIFIED=False;B1R5_RUNTIME_BASELINE=CLOSED_PASS"
 ...
 $before=(Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant();W "SOURCE_SHA256_BEFORE=$before";if($before-cne $Pre -and $before-cne $Post){throw "dragonriding source is not locked B1R5 or B2 image"};$first=($before-ceq $Pre)
 ...
 $com=$env:ComSpec;if(-not $com){throw "ComSpec missing"};$NativeSelfTestArgs=@('/d','/c','echo G17B2_NATIVE_STDOUT& echo G17B2_NATIVE_STDERR 1>&2& exit /b 0');$rc=Invoke-NativeLogged -FilePath $com -NativeArgs $NativeSelfTestArgs -Prefix 'NATIVE_SELFTEST';W "NATIVE_SELFTEST_EXIT=$rc";if($rc-ne 0){throw "native runner selftest failed"}
 ...
 $after=(Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant();W "SOURCE_SHA256_AFTER=$after";if($after-cne $Post){throw "B2 postimage SHA mismatch"};W "G17B2_SOURCE_APPLY_GATE=PASS"
 ...
 if($AfterExeUtc-le $BeforeExeUtc -or $AfterPdbUtc-le $BeforePdbUtc){throw "exe/pdb timestamp did not advance"};if($AfterExeUtc-lt $start -or $AfterPdbUtc-lt $start){throw "exe/pdb predates build start"};if($first -and $aeh-ceq $beh){throw "first B2 build did not change worldserver.exe SHA"}
 W "G17B2_WINDOWS_BUILD_RESULT=PASS";W "STOP_DO_NOT_RUN_SQL_OR_MODIFY_CLIENT";W "NEXT=Start worldserver normally; verify skill3 control, 1200 percent momentum, stall recovery, and typed no-parachute landing";W "RESULT_FILE=$Result";exit 0
}catch{W ("G17B2_WINDOWS_BUILD_ERROR="+$_.Exception.Message);W "G17B2_WINDOWS_BUILD_RESULT=FAIL";W "STOP_DO_NOT_START_WORLDSERVER";W "RESULT_FILE=$Result";exit 1}
