...
constexpr uint32 SPELL_SAFE_LANDING   = 53208; // Parachute effect is always suppressed for G17.
...
constexpr uint32 VISUAL_KIT_BEAST_POUNCE = 14475;    // Charge trail + landing dust.
...
public:
    ...
    bool Execute(uint64 /*execTime*/, uint32 /*diff*/) override
    {
        ...
        if (hasControl)
        {
            ...
            if (WorldSession* session = _player->GetSession())
                ChatHandler(session).SendSysMessage(
                    "|cff80dfff[G17-B2] 完整御空已就绪：1龙息、2动量加速、3向前爬升、4无降落伞特色着陆；速度250%-1200%。|r");
            return true;
        }
        ...
        if (WorldSession* session = _player->GetSession())
            ChatHandler(session).SendSysMessage(
                "|cffff4040[G17-B2] 异步入座或控制权建立失败；worldserver已记录G17B1精确诊断。|r");
        return true;
    }
    ...
    uint32 sourceDisplay, uint32 archetype, ChatHandler* handler)
{
    ...
    if (handler)
        handler->SendSysMessage("|cff80dfff[G17-B2] 正在建立全坐骑原生载具控制权……|r");
    return true;
}
...
struct npc_g17_dragonriding_vehicle : public VehicleAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (WorldSession* session = player->GetSession())
            ChatHandler(session).SendSysMessage(
                "|cffff8040[G17-B2] 已进入禁飞/城市/室内/副本区域，御空状态已无降落伞归一化清理。|r");
    ...
    void DoAction(int32 action) override
    {
        ...
        if (action == ACTION_ACCELERATE)
        {
            ...
            if (_stalling)
                RecoverFromStall();
            SendToRider(me, "|cff80dfff[G17-B2] 动量推进已启动：平滑加速、最高封顶1200%，重复施放不会叠加上限。|r");
            return;
        }
...
private:
        ...
        if (Player* player = GetRider(me))
            if (WorldSession* session = player->GetSession())
                ChatHandler(session).PSendSysMessage(
                    "|cff80dfff[G17-B2] 动量档位 %u/7：目标飞行速度 %u%%（当前动量%u%%）。|r",
    ...
    void EnterStall()
    {
        ...
        G17Dragonriding::SendToRider(me,
            "|cffffb040[G17-B2] 低动量失速：载具开始下坠；向前、俯冲或使用技能2即可恢复。|r");
    }
    ...
    void RecoverFromStall()
    {
        ...
        ApplyMovementRates(_currentSpeedRate, true);
        G17Dragonriding::SendToRider(me,
            "|cff80ff80[G17-B2] 已从失速恢复：上升、下降、转向和前进控制全部恢复。|r");
    }
    ...
    void StartForwardClimb()
    {
        ...
        if (forwardDot <= 1.0f)
        {
            SendToRider(me, "|cffffb040[G17-B2] 技能3前方被阻挡：为避免穿墙已取消本次短爬升，方向不会反转。|r");
            RestoreClientFlightControl(true);
            return;
        }
        ...
        SendToRider(me, "|cff80dfff[G17-B2] 技能3：沿当前朝向向前并向上爬升；短样条结束后立即交还完整控制。|r");
    }
    ...
    void CompleteClimb()
    {
        ...
        ApplyMovementRates(_currentSpeedRate, true);
        G17Dragonriding::SendToRider(me,
            "|cff80ff80[G17-B2] 爬升完成：飞行姿态、重力、速度及全部客户端控制已显式恢复。|r");
    }
    ...
    void StartTypedLanding()
    {
        ...
        {
            SendToRider(me, "|cffff4040[G17-B2] 当前下方没有可验证的安全地面，已取消着陆。|r");
            return;
        }
        ...
        switch (_archetype)
        {
            case ARCHETYPE_MAGIC:
                ...
                SendToRider(me, "|cffc080ff[G17-B2] 魔法/风系着陆：旋风托举并平滑卸除高度。|r");
            ...
            case ARCHETYPE_MECHANICAL:
                ...
                SendToRider(me, "|cffffa040[G17-B2] 机械火箭着陆：反推火焰高速制动，地面自动断推。|r");
                me->GetMotionMaster()->MoveLand(POINT_LAND, destination, 28.0f);
                break;
            case ARCHETYPE_BEAST:
                SendToRider(me, "|cffffd080[G17-B2] 野兽跃落：先接近地面，再向前扑跃安全落地。|r");
            ...
            case ARCHETYPE_DRAGON:
                SendToRider(me, "|cff80dfff[G17-B2] 龙类收翼着陆：受控俯降并以翼压稳定触地。|r");
                me->GetMotionMaster()->MoveLand(POINT_LAND, destination, 18.0f);
                break;
            default:
                SendToRider(me, "|cff80dfff[G17-B2] 通用受控着陆：验证地面后平滑下降。|r");
...
class spell_g17_dragon_safe_landing : public SpellScript
{
    ...
    void StartLanding(SpellEffIndex effectIndex)
    {
        ...
        // Spell 53208 is retained as the existing vehicle-bar button only.
        // Its parachute aura is never applied to the vehicle or rider.
        PreventHitDefaultEffect(effectIndex);
        if (Creature* dragon = GetCaster()->ToCreature())
            if (dragon->IsAIEnabled())
...
public:
    ...
    void OnUpdateZone(Player* player, uint32 /*newZone*/, uint32 /*newArea*/) override
    {
        ...
        {
            if (WorldSession* session = player->GetSession())
                ChatHandler(session).SendSysMessage("|cffff8040[G17-B2] 已进入禁飞/城市/室内/副本区域，御龙载具已安全清理。|r");
            G17Dragonriding::CleanupPlayer(player, false, true);
        }
    }
};
...
public:
    ...
    static bool HandleSummon(ChatHandler* handler)
    {
        ...
        if (player->IsMounted())
        {
            handler->SendSysMessage("|cffff4040[G17-B2] 已在普通坐骑上；请直接重新点击已学坐骑让自动接管工作。|r");
            return true;
        }

        if (!G17Dragonriding::SpawnTypedVehicle(player, 0, 0, 0,
            G17Dragonriding::ARCHETYPE_DRAGON, handler))
            handler->SendSysMessage("|cffff4040[G17-B2] 当前状态或地点不允许建立御龙载具。|r");
        return true;
    }
    ...
    static bool HandleMount(ChatHandler* handler, uint32 spellId)
    {
        ...
        {
            handler->SendSysMessage("|cffff4040[G17-B2] 必须提供自己已经学会的坐骑法术ID（支持包装法术）。|r");
            return true;
        }

        G17Dragonriding::SetAutoEnabled(player, true);
        player->CastSpell(player, spellId, false);
        handler->PSendSysMessage("|cff80dfff[G17-B2] 已施放拥有的坐骑法术%u；成功上马后会自动接管。|r", spellId);
        return true;
    }
    ...
    static bool HandleAuto(ChatHandler* handler, std::string mode)
    {
        ...
        {
            handler->PSendSysMessage("|cffffff00[G17-B2] auto=%s；用法：.dragon auto on|off|r",
                G17Dragonriding::IsAutoEnabled(player) ? "on" : "off");
            return true;
        }

        handler->PSendSysMessage("|cff80dfff[G17-B2] 全坐骑自动接管已%s。|r",
    ...
    static bool HandleDismiss(ChatHandler* handler)
    {
        ...
        {
            handler->SendSysMessage("|cffffff00[G17-B2] 当前没有御龙载具。|r");
            return true;
        }

        G17Dragonriding::CleanupPlayer(player, true);
        handler->SendSysMessage("|cff80dfff[G17-B2] 御龙载具已安全解除。|r");
        return true;
    }
    ...
    static bool HandleStatus(ChatHandler* handler)
    {
        ...
        if (Creature* dragon = G17Dragonriding::GetDragon(player))
        {
            ...
            handler->PSendSysMessage("|cff80dfff[G17-B2] ACTIVE sourceSpell=%u sourceCreature=%u display=%u type=%s vehicle=%u seat=%d movementSeat=%d controlled=%s energy=%u/%u momentum=%u%% speed=%u%% state=%u auto=%s map=%u zone=%u area=%u|r",
            ...
            handler->PSendSysMessage("|cffffff00[G17-B2] INACTIVE auto=%s area_allowed=%s|r",
                G17Dragonriding::IsAutoEnabled(player) ? "on" : "off",
