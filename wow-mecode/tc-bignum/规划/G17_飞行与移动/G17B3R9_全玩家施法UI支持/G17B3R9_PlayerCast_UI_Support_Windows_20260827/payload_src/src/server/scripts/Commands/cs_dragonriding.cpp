...
void WriteMovementPage(Creature* dragon)
{
    ...
    dragon->m_spells[5] = SPELL_PAGE_SWITCH;  // 切换技能页 - B3-R8: STAYS at slot 6 so the
                                              // stock 6-button bar ALWAYS keeps the switch
                                              // (B3-R7 moved it to slot 7 which vanished when
                                              // the client only fills/renders 6 bonus slots)
    dragon->m_spells[6] = SPELL_GLIDE_BRAKE;  // B3-R8: 滑翔/制动 at slot 7 - G17DragonBar
                                              // shows it (native bonus slot or its player-cast
                                              // secure-button fallback by spell ID)
    dragon->m_spells[7] = 0;
}
...
void WriteCombatPage(Creature* dragon, uint32 mountArchetype)
{
    ...
    dragon->m_spells[5] = SPELL_PAGE_SWITCH;  // 切换技能页 - B3-R8: always visible at slot 6
    dragon->m_spells[6] = SPELL_GLIDE_BRAKE;  // B3-R8: 滑翔/制动 at slot 7 (G17DragonBar)
    dragon->m_spells[7] = 0;
}
...
void AddSC_dragonriding_commandscript()
{
    ...
    TC_LOG_INFO("server.loading", " ");
    TC_LOG_INFO("server.loading", ">> G17-B3R9 dragonriding LOADED  build=20260827-r9 (dual-caster movement skills + custom-UI player-cast path)");
