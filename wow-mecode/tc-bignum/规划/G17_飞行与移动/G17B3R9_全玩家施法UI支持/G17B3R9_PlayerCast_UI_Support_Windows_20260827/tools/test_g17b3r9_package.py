...
TOOL = PKG / "tools" / "apply_g17b3r9_source.py"
INSTALL_PS1 = PKG / "Install-Build-G17B3R9-Windows.ps1"
...
POST = "f0564c5ad225a67f0e49477d31d6939d32b490e6d69b8218f122bc7dce5560c3"
...
def t1_lineage():
    ok("T1 original == B3R8 preimage", sha(PKG / "original.cpp") == PRE_B3R8)
...
def t2_gate_simulation():
    ...
    ok("T2 14 upgradeable intermediates incl. B3R6/R7/R8", len(inter) == 14 and PRE_B3R6 in inter and PRE_B3R7 in inter and PRE_B3R8 in inter)
    ok("T2 PS1 fingerprint", "f0564c5a" in ps1 and "G17B3R9_WINDOWS_BUILD_RESULT" in ps1)
...
def t4_payload_content():
    ...
    ok("T4 page switch at slot 6 on BOTH pages (B3R8 heritage)", payload.count("m_spells[5] = SPELL_PAGE_SWITCH;") == 2)
    ok("T4 brake at slot 7 on BOTH pages", payload.count("m_spells[6] = SPELL_GLIDE_BRAKE;") == 2)
    ok("T4 runtime attribute sanitize present",
    ...
    ok("T4 dual-caster handlers (3 movement skills)",
    ...
    ok("T4 diff bounded (<= 12 hunks)", len(hunks) <= 14, f"hunks={len(hunks)}: {hunks}")
    added = [l for l in diff if l.startswith("+") and not l.startswith("+++")]
    removed = [l for l in diff if l.startswith("-") and not l.startswith("---")]
    # bounded diff: header note (5) + 2 page layouts (2+2) + helper (~28) + 2 call sites (2) = small
    ok("T4 bounded diff (added<=80, removed<=30)", len(added) <= 80 and len(removed) <= 30,
...
def main() -> int:
    print(f"G17B3R9_PACKAGE_SELFTEST")
    t1_lineage()
    t2_gate_simulation()
    t3_functional_check()
    t4_payload_content()
    t5_static()
    t6_sums()
    print(f"G17B3R9_PACKAGE_SELFTEST={'PASS' if FAIL == 0 else 'FAIL'} PASS={PASS} FAIL={FAIL}")
