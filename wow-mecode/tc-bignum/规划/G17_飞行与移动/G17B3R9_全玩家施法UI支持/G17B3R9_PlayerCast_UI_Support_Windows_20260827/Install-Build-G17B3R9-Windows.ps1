...
# G17-B3R9: full player-cast support for the custom G17 DragonRide UI.
#   The new custom client UI casts every dragonriding skill as the PLAYER via
#   secure spell buttons.  Combat carriers + 4 G17 carriers already accepted
#   player casters; B3R9 extends the dual-caster path to the three REAL
#   movement skills (55215 boost / 52197 dash / 52226 landing: handlers now
#   use ResolveDragonFromCaster), applies the dragon-energy gate to both
#   cast paths (CheckEnergyCast), and sets CASTABLE_WHILE_MOUNTED at runtime
#   for 55215/52197 (proven B2R3 const_cast pattern; 52226 already has it).
# Works from B3R6 / B3R7 / B3R8 source states. Payload f0564c5a, rollback 3fdb46e8.
param(
...
$Result = Join-Path $UploadDir "G17B3R9_WINDOWS_BUILD_RESULT.txt"
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$Target = Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp"
$Tool = Join-Path $PSScriptRoot "tools\apply_g17b3r9_source.py"
...
try {
    ...
    W "G17B3R9_SOURCE_APPLY_GATE=PASS"
    ...
    W "G17B3R9_WINDOWS_BUILD_RESULT=PASS"
    W "PROOF_OF_LOAD=start worldserver and look for 'G17-B3R6 performance fix LOADED'"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B3R9_WINDOWS_BUILD_ERROR=" + $_.Exception.Message)
    W "G17B3R9_WINDOWS_BUILD_RESULT=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
