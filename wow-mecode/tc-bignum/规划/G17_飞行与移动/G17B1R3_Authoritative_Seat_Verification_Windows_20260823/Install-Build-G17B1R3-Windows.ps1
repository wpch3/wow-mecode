...
$UploadDir=Join-Path $Workspace "uploads";$Result=Join-Path $UploadDir "G17B1R3_WINDOWS_BUILD_RESULT.txt";$Utf8NoBom=New-Object System.Text.UTF8Encoding($false)
$Target=Join-Path $SourceRoot "src\server\scripts\Commands\cs_dragonriding.cpp";$Tool=Join-Path $PSScriptRoot "tools\apply_g17b1r3_source.py"
...
$Pre="2c7594d0f1428a767570063ac90c5f816991bf1d883fe61e45a1a28902a68199";$Post="94ff80334783e8883f0811a1a7f76595d91b729cc43684f00273abb9d955628b"
...
try{
 W "G17B1R3_WINDOWS_BUILD_START";W "SCOPE=AUTHORITATIVE_VEHICLE_SEAT_VERIFICATION";W "RUNS_SQL=False";W "MODIFIES_CLIENT=False";W "R5_CLIENT_STATE_MODIFIED=False"
 ...
 $before=(Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant();W "SOURCE_SHA256_BEFORE=$before";if($before-cne $Pre -and $before-cne $Post){throw "dragonriding source is not locked B1 or B1R3 image"};$first=($before-ceq $Pre)
 ...
 $com=$env:ComSpec;if(-not $com){throw "ComSpec missing"};$NativeSelfTestArgs=@('/d','/c','echo G17B1R3_NATIVE_STDOUT& echo G17B1R3_NATIVE_STDERR 1>&2& exit /b 0');$rc=Invoke-NativeLogged -FilePath $com -NativeArgs $NativeSelfTestArgs -Prefix 'NATIVE_SELFTEST';W "NATIVE_SELFTEST_EXIT=$rc";if($rc-ne 0){throw "native runner selftest failed"}
 ...
 $after=(Get-FileHash -LiteralPath $Target -Algorithm SHA256).Hash.ToLowerInvariant();W "SOURCE_SHA256_AFTER=$after";if($after-cne $Post){throw "B1R3 postimage SHA mismatch"};W "G17B1R3_SOURCE_APPLY_GATE=PASS"
 ...
 if($AfterExeUtc-le $BeforeExeUtc -or $AfterPdbUtc-le $BeforePdbUtc){throw "exe/pdb timestamp did not advance"};if($AfterExeUtc-lt $start -or $AfterPdbUtc-lt $start){throw "exe/pdb predates build start"};if($first -and $aeh-ceq $beh){throw "first B1R3 build did not change worldserver.exe SHA"}
 W "G17B1R3_WINDOWS_BUILD_RESULT=PASS";W "STOP_DO_NOT_RUN_SQL";W "NEXT=Start worldserver normally; test ground mount, 59961, indoor cleanup";W "RESULT_FILE=$Result";exit 0
}catch{W ("G17B1R3_WINDOWS_BUILD_ERROR="+$_.Exception.Message);W "G17B1R3_WINDOWS_BUILD_RESULT=FAIL";W "STOP_DO_NOT_START_WORLDSERVER";W "RESULT_FILE=$Result";exit 1}
