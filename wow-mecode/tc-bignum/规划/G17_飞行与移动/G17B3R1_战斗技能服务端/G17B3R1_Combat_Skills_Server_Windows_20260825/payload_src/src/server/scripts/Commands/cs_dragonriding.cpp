...
void GrantCombatSkills(Player* player, uint32 mountArchetype)
{
    if (!player)
        return;

    // Determine table block for the archetype (login/board re-grant is safe).
    uint32 block = 4; // GENERIC fallback
    switch (mountArchetype)
    {
        case ARCHETYPE_DRAGON:     block = 0; break;
        case ARCHETYPE_BEAST:      block = 1; break;
        case ARCHETYPE_MAGIC:      block = 2; break;
        case ARCHETYPE_MECHANICAL: block = 3; break;
        default:                   block = 4; break;
    }
    for (uint32 i = 0; i < 5; ++i)
    {
        uint32 const sid = COMBAT_SPELL_BASE + block * 5u + i;
        if (!player->HasSpell(sid))
            player->LearnSpell(sid, false);   // custom carriers only exist in G17 tab
    }
    TC_LOG_INFO("scripts.g17.dragonriding",
        "G17B3R1 combat page granted: player={}({}) archetypeBlock=%u",
...
struct npc_g17_dragonriding_vehicle : public VehicleAI
{
    ...
    void PassengerBoarded(Unit* passenger, int8 seatId, bool apply) override
    {
        ...
        if (apply)
        {
            ...
            me->SetMaxPower(POWER_ENERGY, 100);
            me->SetPower(POWER_ENERGY, 100);
            // B3-R1: grant the archetype combat page in the player spellbook.
            G17Dragonriding::GrantCombatSkills(player, GetData(G17Dragonriding::DATA_ARCHETYPE));
            return;
        }
...
// validates context/archetype/energy/battleground.  The actual effect runs
// in the PlayerScript OnSpellCast below (full access to player + spell
// targets), which is the same proven API family as the movement skills.
class spell_g17_combat_skill : public SpellScript
{
    ...
    SpellCastResult CheckCast()
    {
        ...
        Creature* dragon = G17Dragonriding::GetDragon(player);
        if (!dragon || !dragon->IsAIEnabled())
            return SPELL_FAILED_NOT_READY;
...
public:
    ...
    void OnSpellCast(Player* player, Spell* spell, bool /*skipCheck*/) override
    {
        ...
        // B3-R1 combat carriers: execute the real effect (OnSpellCast = after
        // a successful cast, same proven hook family as the movement skills).
        if (G17Dragonriding::IsCombatSkill(spell->GetSpellInfo()->Id))
        {
            G17Dragonriding::HandleCombatSkillSpell(player, spell);
            return;
        }
