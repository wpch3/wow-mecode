...
PACKAGE = PLAN / "G17B0_Windows_Build_v3_ASCII_NativeSafe_20260822"
ZIP = PLAN / "G17B0_Windows_Build_v3_ASCII_NativeSafe_20260822.zip"
TOP = PACKAGE.name
ZIP_SIZE = 9665
ZIP_SHA256 = "114054be599fcaec840b7fc9798d020f5e58cd5b7b11bcbf06a509498eacf946"
...
for token in (
    ...
    "G17B0_NATIVE_STDERR 1>&2",
    "NATIVE_RUNNER_SELFTEST_EXIT=",
    "G17B0_NATIVE_RUNNER_SELFTEST=PASS",
    "$CMakeExit = Invoke-NativeLogged",
    "$BuildExit = Invoke-NativeLogged",
    "G17B0_WINDOWS_BUILD_V3_NATIVE_SAFE_START",
):
    require(token in ps1, f"native-safe contract: {token}")
require("& $CMake " not in ps1 and "& $MSBuild " not in ps1,
        "CMake/MSBuild must not use direct native pipelines")
continue_pos = ps1.index('$ErrorActionPreference = "Continue"')
output_pos = ps1.index("$NativeOutput = @(& $FilePath @NativeArgs 2>&1)")
exit_pos = ps1.index("$NativeExit = $LASTEXITCODE")
restore_pos = ps1.index("$ErrorActionPreference = $SavedErrorActionPreference")
require(continue_pos < output_pos < exit_pos < restore_pos,
        "native output/exit capture and preference restore order")
...
for token in (
    ...
    'cmake.exe', '$CMakeArgs = @("-S", $SourceRoot, "-B", $BuildRoot)',
...
print("G17B0_BUILD_V3_PS1_ASCII_ONLY=PASS")
print("G17B0_BUILD_V3_CMD_ASCII_ONLY=PASS")
print("G17B0_WINDOWS_BUILD_V3_SCRIPT_LEXICAL=PASS")
print("G17B0_WINDOWS_BUILD_V3_SOURCE_GATES=PASS")
print("G17B0_WINDOWS_BUILD_V3_CMAKE_MEMBERSHIP_GATE=PASS")
print("G17B0_WINDOWS_BUILD_V3_FRESH_OBJECT_GATE=PASS")
print("G17B0_WINDOWS_BUILD_V3_NO_SERVER_START=PASS")
print("G17B0_WINDOWS_BUILD_V3_NATIVE_STDERR_POLICY=PASS")
print("G17B0_WINDOWS_BUILD_V3_NATIVE_RUNNER_SELFTEST=PASS")
print("G17B0_WINDOWS_BUILD_V3_INTERNAL_CHECKSUMS=PASS_7")
print("G17B0_WINDOWS_BUILD_V3_NATIVE_SAFE_ZIP=FINAL_PASS")
