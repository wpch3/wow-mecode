-- G23-P3A login welcome and rank-aware shared command registry view.
...
local function showCommands(player)
    ...
    for _, entry in ipairs(G23.GetHelpEntries(G23.GetRank(player))) do
    ...
    line(player)
    say(player, "|cff888888服务器总入口：.server；GM完整帮助：.gmhelp；诊断：.luadiag。|r")
end
...
local function onLogin(event, player)
    ...
    say(player, "  输入 |cffffff00.server|r 打开服务器助手，或用 |cffffff00.help2|r 查看指令")
    line(player)
end
...
print("[G23-P3A] custom_welcome.lua 已加载 -- 权限过滤共享命令表驱动.help2")
