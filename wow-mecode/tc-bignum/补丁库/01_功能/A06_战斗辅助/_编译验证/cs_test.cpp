...
namespace CombatHelper
{
    ...
    struct ComboState
    {
        ...
        uint32 buffQueueUntil = 0;                    // .buff 队列跑到什么时候(ms)，期间 combo 完全让路
        uint32 buffGeneration = 0;                   // F44：取消旧.buff队列事件

        // F44R1：临时部署物按“每次进入战斗一次”管理，不依赖不存在的Unit Aura。
        bool wasInCombat = false;
        std::unordered_set<uint32> deployedThisCombat;
     ...
     *   返回  0 = 身上没有
     *
     * F44R1用它执行严格生命周期：返回值只要不是0，就仍视为有效，
     * 不设置任何提前刷新窗口；持续时间真正归零/光环移除后才允许补。
     *
    ...
    inline void InvalidatePlayerRuntime(Player* player, bool eraseState)
    {
        ...
        if (it != States().end())
        {
            ...
            it->second.wasInCombat = false;
            it->second.deployedThisCombat.clear();
            if (eraseState)
                States().erase(it);
        }
    ...
    inline bool IsPaladinBlessing(uint32 rank1)
    {
        return rank1 == 25782 || rank1 == 25894 || rank1 == 25898 || rank1 == 20911;
    }

    /*
     * 同一圣骑对同一目标只能维持一种祝福；强效祝福还会覆盖同职业成员。
     * 所以先按“目标职业”统一决策：该职业有坦克就庇护/王者，纯法系优先
     * 智慧，纯物理优先力量。只在候选确实已学会时选择，避免空缺。
     */
    inline uint32 PreferredPaladinBlessing(Player* player, Unit* target, float maxDist)
    {
        if (!player || !target)
            return 0;

        bool classHasTank = false;
        bool classUsesMana = false;
        uint8 targetClass = target->GetClass();
        ForEachFriendlyGroupUnit(player, [&](Unit* unit, bool tankish, bool)
        {
            if (unit->GetClass() != targetClass ||
                !IsLegalFriendlyGroupTarget(player, unit, maxDist))
                return;
            classHasTank = classHasTank || tankish;
            classUsesMana = classUsesMana || unit->GetPowerType() == POWER_MANA;
        });

        uint32 tankOrder[] = { 20911, 25898, 25782, 25894 };
        uint32 manaOrder[] = { 25894, 25898, 25782, 20911 };
        uint32 physicalOrder[] = { 25782, 25898, 20911, 25894 };
        uint32* order = classHasTank ? tankOrder : (classUsesMana ? manaOrder : physicalOrder);
        for (uint8 i = 0; i < 4; ++i)
            if (ResolveRank(player, order[i]))
                return order[i];
        return 0;
    }

    inline bool IsPreferredAutoBuffTarget(Player* player, CombatSpec::Skill const& skill,
                                          Unit* target, float maxDist)
    {
        if (!player || !target)
            return false;
        if (skill.flags & (CombatSpec::SF_MANUAL | CombatSpec::SF_DEPLOYABLE |
                           CombatSpec::SF_COMBAT_UTILITY))
            return false;
        if ((skill.flags & CombatSpec::SF_SELF) && target != player)
            return false;
        if ((skill.flags & (CombatSpec::SF_RAID_BUFF | CombatSpec::SF_MAINTAIN_FRIEND)) &&
            !IsLegalFriendlyGroupTarget(player, target, maxDist))
            return false;
        if (IsPaladinBlessing(skill.spell) &&
            PreferredPaladinBlessing(player, target, maxDist) != skill.spell)
            return false;
        return true;
    }
                                   ...
                                   float maxDist, std::vector<Unit*>& out)
    {
        ...
        // F44R1硬规则：有任何剩余时间（包括永久Aura）都不补；仅Aura真正消失后补。
        auto isMissing = [&](Unit* unit) -> bool
        {
            return IsPreferredAutoBuffTarget(player, skill, unit, maxDist) &&
                   AuraRemainMs(unit, skill.spell, caster) == 0;
        };
        ...
        if ((skill.flags & CombatSpec::SF_SELF) && isMissing(player))
...
public:
    ...
    static bool HandleBuffCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!tok.empty())
        {
            ...
            {
                st.autoBuff = false;
                ++st.buffGeneration;      // 立即使尚未执行的.buff now事件失效
                st.buffQueueUntil = 0;
                handler->PSendSysMessage("|cffff8000[自动增益]|r 已关闭，待执行增益队列已取消");
                return true;
            }
            if (tok[0] == "dispel")
            {
    ...
    static bool BuffNow(ChatHandler* handler, Player* player)
    {
        ...
        if (nowMs < st.buffQueueUntil)
        {
            handler->PSendSysMessage("|cffff8000[增益]|r 现有补增益队列仍在执行；不会重建或重复施放。");
            return true;
        }
        ...
        for (BuffCast cast : queue)
        {
            player->m_Events.AddEventAtOffset([guid, cast, buffGeneration]()
            {
                ...
                CombatSpec::Skill liveSkill { cast.rank1, cast.flags, "" };
                if (!CombatHelper::IsPreferredAutoBuffTarget(p, liveSkill, target, 40.0f))
                    return;

                ObjectGuid auraCaster = (cast.flags & CombatSpec::SF_MAINTAIN_FRIEND)
                    ? p->GetGUID() : ObjectGuid::Empty;
                // 队列执行时必须再次检查；Aura只要还存在一毫秒就绝不提前刷新。
                if (CombatHelper::AuraRemainMs(target, cast.rank1, auraCaster) != 0)
                    return;
    ...
    {
        if (sk.flags & CombatSpec::SF_MANUAL)
            return 2; // 技能栏可用，但自动路径永远不碰物品目标/手动技能
        ...
        // --- 目标类型：任何友方意图绝不能退化成敌方或自己兜底 ---
        if ((sk.flags & (CombatSpec::SF_FRIEND | CombatSpec::SF_MAINTAIN_FRIEND)) &&
        ...
        if (sk.flags & CombatSpec::SF_BUFF_KEEP)
        {
            /*
             * F44R1严格规则：
             *   -1 = 永久光环，绝不重上；>0 = 尚有任意持续时间，绝不提前刷新；
             *    0 = rank链上已完全没有该Aura，此时才允许补。
             */
            if (!target)
                return 2;
            ...
            ObjectGuid auraCaster = (sk.flags & CombatSpec::SF_MAINTAIN_FRIEND)
                ? player->GetGUID() : ObjectGuid::Empty;
            // F44R1：永久或有任何剩余持续时间都不补；只允许Aura完全消失后的施法。
            if (CombatHelper::AuraRemainMs(target, sk.spell, auraCaster) != 0)
                return 2;
        }

        // 蝰蛇守护等条件式互斥状态虽不走长期buff维护，也不能每个GCD重放。
        if ((sk.flags & CombatSpec::SF_EXCLUSIVE_BUFF) &&
            !(sk.flags & CombatSpec::SF_BUFF_KEEP) && target &&
            CombatHelper::HasAnyRankAura(target, sk.spell))
            return 2;
    ...
    static void ComboTick(Player* player)
    {
        ...
        /*
         * F44R1生命周期闸：
         *   - 每次真正进入战斗，临时部署物获得一套新的“一次成功”额度；
         *   - .buff now排队期间，脱战时连招完全让出GCD；
         *   - 一旦进战斗或治疗职责发现需治疗血线，立即取消剩余buff队列，
         *     绝不让补增益饿死治疗/战斗。
         */
        CombatHelper::ComboState& lifecycle = CombatHelper::St(player);
        bool inCombatNow = player->IsInCombat();
        if (inCombatNow && !lifecycle.wasInCombat)
            lifecycle.deployedThisCombat.clear();
        lifecycle.wasInCombat = inCombatNow;

        if (GameTime::GetGameTimeMS() < lifecycle.buffQueueUntil)
        {
            bool healingMustPreempt = false;
            if (role == CombatSpec::ROLE_HEALER)
            {
                CombatHelper::HealTarget qht;
                CombatHelper::PickHealTarget(player, 40.0f, qht);
                if (qht.unit)
                {
                    uint32 line = (qht.unit == player) ? tune.healSelfPct : tune.healTargetPct;
                    healingMustPreempt = qht.hpPct <= float(line);
                }
            }

            if (inCombatNow || healingMustPreempt)
            {
                ++lifecycle.buffGeneration;
                lifecycle.buffQueueUntil = 0;
            }
            else
                return;
        }
         ...
         *   其他职责  -> 只在自己血少时自奶
         */
        bool healerWaitingForHeal = false;
        {
            ...
            if (ht.unit)
            {
                if (isHealer)
                {
                    uint32 line = (ht.unit == player) ? tune.healSelfPct : tune.healTargetPct;
                    healerWaitingForHeal = ht.hpPct <= float(line);
                }
                ...
                for (CombatSpec::Skill const& sk : plan.core)
                {
                    ...
                    /*
                     * 自身施放的治疗准备技（自然迅捷/潮汐之力）仍应按最低血友方
                     * 的紧急线触发，只把实际施法目标改为自己；不能改用自己的血线。
                     */
                    Unit* healCastTarget = (sk.flags & CombatSpec::SF_SELF) ? player : ht.unit;
                    uint8 r = CheckHealSkill(player, ht, sk, real, tune);
                    if (r == 1) { blockedByMove = true; continue; }
                    if (r != 0) continue;

                    if (!CombatHelper::CastChecked(player, healCastTarget, real))
                        continue;
        ...
        // 治疗职责发现血线不安全时，只允许必要的回蓝动作；即使当前GCD/CD/移动
        // 导致没奶出来，也绝不下落到攻击技能抢占下一个施法窗口。
        if (healerWaitingForHeal)
        {
            for (CombatSpec::Skill const& sk : plan.utility)
                if ((sk.flags & CombatSpec::SF_MANA_LOW) && tryCast(sk, player))
                    return;
            return;
        }
        ...
        if (inCombatNow)
        {
            // 误导/嫁祸：稳定锁定坦克，按法术CD使用；不靠短暂Aura伪装成长效buff。
            if (target)
                for (CombatSpec::Skill const& sk : plan.utility)
                {
                    if (!(sk.flags & CombatSpec::SF_COMBAT_UTILITY))
                        continue;
                    Unit* friendTarget = CombatHelper::FindMaintainedTarget(player, sk.spell, 40.0f);
                    if (friendTarget && tryCast(sk, friendTarget))
                        return;
                }

            // 图腾等部署物没有同ID的Unit Aura：一次进战斗只记录一次成功施放。
            CombatHelper::ComboState& ds = CombatHelper::St(player);
            for (CombatSpec::Skill const& sk : plan.opener)
            {
                if (!(sk.flags & CombatSpec::SF_DEPLOYABLE) ||
                    ds.deployedThisCombat.count(sk.spell))
                    continue;
                if (tryCast(sk, player))
                {
                    ds.deployedThisCombat.insert(sk.spell);
                    return;
                }
            }
        }
        ...
        // ==============================================================
        /*
         * F44R1：.buff队列的全局让路已在tick入口完成；这里每2秒只做一次缺失扫描。
         * 扫描频率不再承担“防重放”职责——真正防线是AuraRemainMs必须严格等于0。
         * 因而无论脱战/战斗、普通/副本场景，都只会在持续时间真正结束后补一次；
         * 治疗、驱散、打断和部署物仍排在它前面。
         */
        if (autoBuff)
        {
            ...
            bool throttled = (bs.lastBuffTry != 0 && nowMs - bs.lastBuffTry < 2000);
        ...
        for (CombatSpec::Skill const& sk : plan.core)
        {
            ...
            if (sk.flags & (CombatSpec::SF_HEAL | CombatSpec::SF_HEAL_AOE |
                            ...
                            CombatSpec::SF_MAINTAIN_FRIEND | CombatSpec::SF_MANUAL |
                            CombatSpec::SF_DEPLOYABLE | CombatSpec::SF_COMBAT_UTILITY))
                continue;
