    ...
    printf("\n===== 问题 2：治疗技能没被使用 =====\n");
    {
        ...
        for (auto const& s : pl.core)
            if (s.flags & (CombatSpec::SF_HEAL | CombatSpec::SF_HOT |
                ...
                ++healCount;
        printf("         动态治疗技总数：%d\n", healCount);
        CHECK(healCount >= 7, "治疗方案有足够多的动态治疗技");

        bool beaconStableOpener = false;
        for (auto const& s : pl.opener)
            if (s.spell == 53563 && (s.flags & CombatSpec::SF_MAINTAIN_FRIEND))
                beaconStableOpener = true;
        CHECK(beaconStableOpener, "圣光道标已从动态治疗移入稳定友方opener");
    }
