...
$Result = Join-Path $UploadDir "G17B2R3_WINDOWS_ROLLBACK_RESULT.txt"
...
try {
    W "G17B2R3_WINDOWS_ROLLBACK_START"
    ...
    W "G17B2R3_WINDOWS_ROLLBACK=PASS"
    W "FLOOR=B2R1 ($B2R1)"
    W "RESULT_FILE=$Result"
    exit 0
} catch {
    W ("G17B2R3_WINDOWS_ROLLBACK_ERROR=" + $_.Exception.Message)
    W "G17B2R3_WINDOWS_ROLLBACK=FAIL"
    W "RESULT_FILE=$Result"
    exit 1
}
