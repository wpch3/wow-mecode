...
SQL = ROOT / "sql/G17B2R3_world_landing_binding_guard.sql"
...
SAFE_ROLLBACK_SHA = "3b92e815dc81ade4aa9927c19716dabddb8e8f93a6d0aff8b32c80dfbcbfc7f1"
...
class TestPowerShellInstaller(unittest.TestCase):
    ...
    def setUp(self):
        self.install = (ROOT / "Install-Build-G17B2R3-Windows.ps1").read_text(encoding="utf-8")
        self.rollback = (ROOT / "Rollback-Build-G17B2R3-Windows.ps1").read_text(encoding="utf-8")
    ...
    def test_05_sql_gate_markers_match_installer(self):
        # Regression: the installer waited for "G17B2R3_SPELL_52226_OVERRIDE=PASS"
        # but the castable SQL ALWAYS emits "G17B2R3_SPELL_52226_CASTABLE=PASS".
        ...
        markers = {
            "G17B2R3_world_landing_binding_guard.sql":
                "G17B2R3_WORLD_BINDING_GUARD=PASS",
            "G17B2R3_world_spell52226_castable_override.sql":
...
class TestWorldBindingGuard(unittest.TestCase):
    def test_01_sql_targets_world_and_52226(self):
        ...
        self.assertRegex(sql, r"G17B2R3_WORLD_BINDING_GUARD")
...
class TestDocsConsistency(unittest.TestCase):
    ...
    def test_02_readmes_use_postimage_not_stale_hash(self):
        ...
        stale_claims = ("postimage=61342067", "postimage=03dd649d",
                        ...
                        "SOURCE_SHA256_AFTER 必须变成 3b92e815",
                        "SOURCE_SHA256_AFTER（应为 3b92e815")
        for doc in self.docs:
            text = doc.read_text(encoding="utf-8")
            self.assertIn("98446106", text, doc.name)
            for stale in stale_claims:
                self.assertNotIn(stale, text, doc.name)


class TestB2R3LandingSanitizer(unittest.TestCase):
    """B2R3: 52226 RequiresSpellFocus=1553 / CasterAuraSpell=52255 reject the
    cast in Spell::CheckCast BEFORE any spell-script hook, and the fork's
    spell_dbc table has no columns for them; the only reliable fix is the
    runtime SpellInfo sanitizer in the payload."""

    def setUp(self):
        self.text = SOURCE.read_text(encoding="utf-8")

    def test_01_sanitizer_function_exists_and_clears_focus(self):
        self.assertIn("void EnsureLandingCommandCastable()", self.text)
        self.assertIn("info->RequiresSpellFocus = 0;", self.text)
        self.assertIn("info->CasterAuraSpell = 0;", self.text)

    def test_02_sanitizer_clears_stances_items_totems(self):
        self.assertIn("info->Stances = 0;", self.text)
        self.assertIn("info->StancesNot = 0;", self.text)
        self.assertIn("info->EquippedItemClass = -1;", self.text)
        self.assertIn("info->EquippedItemInventoryTypeMask = 0;", self.text)
        self.assertIn("info->Reagent", self.text)
        self.assertIn("info->TotemCategory", self.text)

    def test_03_sanitizer_called_at_load_and_boarding(self):
        # AddSC runs at script load; PassengerBoarded(apply=true) is the belt
        # in case Scripts load before SpellInfo.  Both must call it.
        self.assertIn("G17Dragonriding::EnsureLandingCommandCastable();", self.text)
        call_count = self.text.count("G17Dragonriding::EnsureLandingCommandCastable();")
        self.assertGreaterEqual(call_count, 2)

    def test_04_sanitizer_proof_of_load_banner(self):
        self.assertIn(
            ">> G17-B2R3 landing command %u cast-gates cleared (focus/aura/item/stance)",
            self.text)

    def test_05_casts_still_bind_to_52226_signature(self):
        # The spell identity is preserved: same script names and same target.
        self.assertIn("spell_g17_dragon_safe_landing", self.text)
        self.assertIn("SPELL_SAFE_LANDING   = 52226", self.text)


class TestB2R3Skill3AntiReverse(unittest.TestCase):
    """B2R3: the dash path must be sampled AFTER the old motion is stopped;
    R2 sampled first and cleared afterwards, so the old (often backward)
    spline was finalized at its back-most endpoint and the client showed a
    short reverse before the dash."""

    def test_01_stop_moving_precedes_path_sample(self):
        text = SOURCE.read_text(encoding="utf-8")
        body = text[text.index("void StartForwardClimb()"):]
        idx_stop = body.index("me->StopMoving();")
        idx_build = body.index("BuildClimbPath(distance, path, exitHeading)")
        self.assertLess(idx_stop, idx_build,
                        "StopMoving must run before BuildClimbPath")

    def test_02_clear_motion_and_facing_before_sample(self):
        text = SOURCE.read_text(encoding="utf-8")
        body = text[text.index("void StartForwardClimb()"):]
        idx_clear = body.index("RestoreClientFlightControl(true);")
        idx_build = body.index("BuildClimbPath(distance, path, exitHeading)")
        self.assertLess(idx_clear, idx_build)

    def test_03_old_late_clear_removed(self):
        # The R2 order had RestoreClientFlightControl(true) AFTER
        # _momentum adjustment and BEFORE the spline launch; R3 moves it
        # before the path build and must not keep a duplicate late clear.
        text = SOURCE.read_text(encoding="utf-8")
        body = text[text.index("void StartForwardClimb()"):]
        after_momentum = body[body.index("_momentum = std::max(0.0f, _momentum - 0.18f);"):]
        idx_launch = after_momentum.index("LaunchMoveSpline")
        self.assertNotIn("RestoreClientFlightControl(true);", after_momentum[:idx_launch])
