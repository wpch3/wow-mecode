/*
 * ============================================================================
 *  CombatSpecData.h —— 31 个专精的技能数据表（F44目标策略版）
 * ============================================================================
 *
 *  数据来源（关键）：
 ...
 *
 *  31 个专精 = 10 职业 × 3 专精 + 德鲁伊第 4 专精（平衡/野性猫/野性熊/恢复）
...
namespace CombatSpec
{
    ...
    enum SkillFlag : uint32
    {
        ...
        // ---- F44：稳定单友方维护（道标/大地盾/警戒/魔法专注等）----
        SF_MAINTAIN_FRIEND = 0x100000, // 锁定合法友方；目标失效才迁移
        SF_FRIEND          = 0x200000, // 动态合法友方（保护/牺牲等非治疗效果）

        // ---- F44R1：生命周期/互斥/非Aura技能分类 ----
        SF_EXCLUSIVE_BUFF  = 0x400000, // 属于互斥家族；全专精计划只保留一个首选
        SF_MANUAL          = 0x800000, // 需物品目标或仅手动场景；只进技能栏，不自动施放
        SF_DEPLOYABLE      = 0x1000000,// 图腾等临时部署物；每次进入战斗只成功部署一次
        SF_COMBAT_UTILITY  = 0x2000000,// 误导/嫁祸等战斗工具；按CD对稳定友方使用
    };
