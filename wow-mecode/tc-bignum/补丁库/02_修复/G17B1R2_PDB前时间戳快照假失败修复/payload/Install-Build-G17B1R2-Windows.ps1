...
$UploadDir=Join-Path $Workspace "uploads";$Result=Join-Path $UploadDir "G17B1R2_WINDOWS_BUILD_RESULT.txt";$Utf8NoBom=New-Object System.Text.UTF8Encoding($false)
...
try{
 W "G17B1R2_WINDOWS_BUILD_START";W "SCOPE=G17B1R2_PDB_TIMESTAMP_SNAPSHOT_FIX_PLUS_G17B1_ALL_MOUNT_INTERCEPT";W "RUNS_SQL=False";W "MODIFIES_CLIENT=False";W "R5_CLIENT_STATE_MODIFIED=False"
 ...
 $com=$env:ComSpec;if(-not $com){throw "ComSpec missing"};$NativeSelfTestArgs=@('/d','/c','echo G17B1R2_NATIVE_STDOUT& echo G17B1R2_NATIVE_STDERR 1>&2& exit /b 0');$rc=Invoke-NativeLogged -FilePath $com -NativeArgs $NativeSelfTestArgs -Prefix 'NATIVE_SELFTEST';W "NATIVE_SELFTEST_EXIT=$rc";if($rc-ne 0){throw "native runner selftest failed"}
 ...
 if($ae.LastWriteTimeUtc-le $BeforeExeUtc -or $ap.LastWriteTimeUtc-le $BeforePdbUtc){throw "exe/pdb timestamp did not advance"};if($first -and $aeh-ceq $beh){throw "first B1 build did not change worldserver.exe SHA"}
 W "G17B1R2_WINDOWS_BUILD_RESULT=PASS";W "STOP_DO_NOT_RUN_SQL";W "NEXT=Start worldserver normally; use normal owned mount buttons with default auto=on";W "RESULT_FILE=$Result";exit 0
}catch{W ("G17B1R2_WINDOWS_BUILD_ERROR="+$_.Exception.Message);W "G17B1R2_WINDOWS_BUILD_RESULT=FAIL";W "STOP_DO_NOT_START_WORLDSERVER";W "RESULT_FILE=$Result";exit 1}
