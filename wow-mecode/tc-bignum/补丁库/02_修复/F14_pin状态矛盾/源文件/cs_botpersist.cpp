...
public:
    ...
    static void CleanupBroken(uint32 entry, bool hasData, bool hasSpawn)
    {
        if (hasSpawn)
        {
            ...
            if (res)
            {
                do
                {
                    ...
                    // Creature.h:222  static void DeleteFromDB(spawnId)  public(78段)
                    // 它会同时清 creature 表和网格
        ...
        if (hasData)
        {
            // botdatamgr.h:217 UpdateNpcBotData public(203段)
    ...
    static bool DoFix(ChatHandler* handler, Player* /*player*/)
    {
        ...
        handler->SendSysMessage("|cff00ff00 已清理孤儿记录|r");
        handler->SendSysMessage("|cffffff00 注意：内存缓存要【重启服务端】才会同步|r");
        handler->SendSysMessage("|cffffff00 重启后它会变回普通游荡bot，那时可以重新 .pin|r");
        return true;
    }
