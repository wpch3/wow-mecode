...
PRE_SHA256='94ff80334783e8883f0811a1a7f76595d91b729cc43684f00273abb9d955628b'
POST_SHA256='e9418704731a2d9cd5119cc2024079a2326802796d00bf24e88928dd17ea7059'
PAYLOAD=Path(__file__).resolve().parents[1]/'payload'/SOURCE_RELATIVE
def sha(p:Path)->str:return hashlib.sha256(p.read_bytes()).hexdigest()
def atomic_write(p:Path,b:bytes)->None:
 t=p.with_name(p.name+'.g17b1r4.tmp')
...
def check(root:Path)->str:
 ...
 print(f'G17B1R4_SOURCE_STATE={s}');print(f'TARGET_SHA256={h}');return s
def apply(root:Path)->None:
 state=check(root);t=root/SOURCE_RELATIVE
 if sha(PAYLOAD)!=POST_SHA256:raise RuntimeError('package payload SHA mismatch')
 if state=='ALREADY_APPLIED':print('G17B1R4_SOURCE_APPLY=ALREADY_CURRENT');return
 bak=t.with_name(t.name+'.g17b1r4.preimage')
 if bak.exists():
 ...
 print(f'BACKUP={bak}');print('G17B1R4_SOURCE_APPLY=PASS')
def rollback(root:Path)->None:
 t=root/SOURCE_RELATIVE;bak=t.with_name(t.name+'.g17b1r4.preimage')
 if not t.is_file() or sha(t)!=POST_SHA256:raise RuntimeError('target is not exact G17B1R4 postimage')
 if not bak.is_file() or sha(bak)!=PRE_SHA256:raise RuntimeError('exact G17B1R4 backup missing')
 atomic_write(t,bak.read_bytes())
 if sha(t)!=PRE_SHA256:raise RuntimeError('rollback SHA mismatch')
 bak.unlink();print('G17B1R4_SOURCE_ROLLBACK=PASS')
def main():
