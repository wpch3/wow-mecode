...
void WorldSession::HandleLootOpcode(WorldPackets::Loot::LootUnit& packet)
{
    ...
    Player* player = GetPlayer();
    player->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_LOOTING);

    // F45: 金币必须在 SendLoot 响应前进入圆心 Loot，客户端才会发送
    // CMSG_LOOT_MONEY。这里只聚合，不加钱；随后仍只走一次原有
    // NPCBot/真人/单人分金链和 OnLootMoney。
    uint32 gatheredGold = 0;
    uint32 gatheredGoldCorpses = 0;
    Creature* aoeOrigin = player->GetMap()->GetCreature(packet.Unit);
    if (CustomAoELoot::Enabled() && aoeOrigin)
    {
        gatheredGold = CustomAoELoot::GatherMoneyAround(
            player, aoeOrigin, &gatheredGoldCorpses);
        aoeOrigin->loot.gold += gatheredGold;
    }

    player->SendLoot(packet.Unit, LOOT_CORPSE);

    // F45唯一物品入口：仅在原版 SendLoot 已完成距离、归属、动态旗标和
    // permission 检查并真正建立该拾取窗后运行；不再依赖成功存入物品槽。
    if (aoeOrigin && player->GetLootGUID() == packet.Unit)
        CustomAoELoot::LootAllAround(
            player, aoeOrigin, gatheredGold, gatheredGoldCorpses);
}
