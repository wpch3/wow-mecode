...
#include "Group.h"              // step55: RemoveMember(210)/AddMember(197)/GetMembersCount(259) 全 public(185段)
#include "botdatamgr.h"         // step55: BotDataMgr::FindBot —— Group.cpp:704 官方就是这么拿bot的
                                // 注: Creature::GetBotGroup 在 Creature.h:397，已由上面的 Creature.h 提供
...
public:
    ...
    // ------------------------------------------------------------------
    // ------------------------------------------------------------------
    //  step55  刷新队伍窗口里的 bot 名字
    // ------------------------------------------------------------------
    //  队伍成员名是 MemberSlot 里的一份快照（Group.cpp:491），改名后不会自动变。
    //  MemberSlot 外部改不了（_getMemberWSlot 是 protected），
    //  所以走官方路径：把 bot 踢出队伍再加回来，让它重新拍快照。
    //
    //  【安全性】
    //    Group.cpp:695  RemoveMember 里 guid.IsCreature() 是官方 npcbot 分支，
    //                   会正确处理 SetBotGroup / DelinkBotMember / 数据库删除
    //    Group.cpp:739  但当 GetMembersCount() <= 2 时【不走那个分支】，
    //                   可能导致队伍解散 -> 所以人数太少时我们【不动】，只提示。
    static void RefreshGroupName(ChatHandler* handler, uint32 entry)
    {
        // botdatamgr.h:232  static Creature const* FindBot(uint32 entry)  public(203段)
        // 返回 const 指针，但 Group::AddMember 要非 const。
        // Group.cpp:704 官方自己也是这么 const_cast 的，照抄它的写法。
        Creature* bot = const_cast<Creature*>(BotDataMgr::FindBot(entry));
        if (!bot)
            return;                     // bot 不在世界里，没有队伍可刷

        Group* grp = bot->GetBotGroup();        // Creature.h:397 public(78段)
        if (!grp)
            return;                     // 不在队伍里，无需处理

        // Group.cpp:701  if (GetMembersCount() > (isBGGroup()||isBFGroup() ? 1u : 2u))
        // 人数不够时 RemoveMember 会走【解散队伍】的路径，
        // 为了改个名字把队伍弄散了得不偿失 -> 这种情况只提示，不动手。
        if (grp->GetMembersCount() <= 2)        // Group.h:259 public(185段)
        {
            handler->SendSysMessage(
                "|cffffff00 队伍人数少于3，为避免队伍被解散，未自动刷新队伍窗口。|r");
            handler->SendSysMessage(
                "|cffffff00 请手动把它踢出队伍再重新邀请，名字就会更新。|r");
            return;
        }
        ...
        // Group.cpp:695 起是 npcbot 专用分支，会正确解绑
        grp->RemoveMember(botGuid);

        // Group.h:197  AddMember(Creature*)  public(185段)
        // Group.cpp:491 会重新 member.name = creature->GetName()
        if (grp->AddMember(bot))
        {
            handler->SendSysMessage("|cff00ff00 队伍窗口的名字已刷新。|r");
        }
        else
        {
            handler->SendSysMessage(
                "|cffff0000 重新入队失败（可能队伍已满）。请手动重新邀请它。|r");
        }
    }
    ...
    static bool RenameNpcBot(ChatHandler* handler, uint32 entry, std::string const& newName, bool chinese)
    {
        ...
        if (chinese)
        {
            ...
            // 热加载，不用重启
            sObjectMgr->LoadCreatureLocales();      // ObjectMgr.h:1156 public(947段)

            // ================================================================
            // step55 【关键补漏】只 LoadCreatureLocales 是不够的
            // ================================================================
            //  客户端头顶的名字走 CMSG_CREATURE_QUERY，服务端回的是
            //  【预生成的缓存包】而不是实时拼的：
            //
            //    QueryHandler.cpp:124
            //      if (sWorld->getBoolConfig(CONFIG_CACHE_DATA_QUERIES))
            //          SendPacket(&ci->QueryData[GetSessionDbLocaleIndex()]);
            //
            //    World.cpp:1586  CacheDataQueries 默认 true  -> 走缓存分支
            //
            //  QueryData 是启动时由 Creature.cpp:190 BuildQueryData 烤好的，
            //  中文名在那时就被烤进去了（:197 GetLocaleString(cl->Name,...)）。
            //  LoadCreatureLocales() 只更新 CreatureLocale 内存表，
            //  【不会重烤 QueryData】-> 客户端拿到的还是旧名字。
            //
            //  官方 .reload creature_template_locale (cs_reload.cpp:1005)
            //  也有同样的问题 —— 它也只调 LoadCreatureLocales()。
            //  而 .reload creature_template (cs_reload.cpp:453) 就记得重烤：
            //      sObjectMgr->InitializeQueriesData(QUERY_DATA_CREATURES);
            //
            //  所以这里必须自己补上这一刀。
            //  ObjectMgr.h:1241  InitializeQueriesData(QueryDataGroup)  public(947段)
            //  ObjectMgr.h:927   QUERY_DATA_CREATURES = 0x01
            sObjectMgr->InitializeQueriesData(QUERY_DATA_CREATURES);
        }
        else
        {
            ...
            // sObjectMgr->LoadCreatureTemplate(fields)（cs_reload.cpp:449）。
            // 我们这里提示用户去执行，比在这儿复刻一遍安全。
            //
            // step55: 但 QueryData 缓存这一刀我们【可以】先补上，
            // 这样即使用户忘了 reload，至少头顶名字这条链不会更糟。
            // （注意：英文名要真正生效仍需 .reload creature_template <entry>，
            //  因为 CreatureTemplate::Name 在内存里还没更新，
            //  重烤缓存用的还是旧 Name。所以下面的提示【不能删】。）
        }
        ...
        // ================================================================
        //  step55 【第二处补漏】队伍窗口的名字是一份【快照】
        // ================================================================
        //  Group.cpp:491  bot 加入队伍那一刻:
        //      member.name = creature->GetName();      <- 拷贝一份存进 MemberSlot
        //  Group.cpp:2049 发包时:
        //      data << citr->name;                     <- 直接用那份快照，不实时查
        //
        //  -> bot 已经在队里时改名，队伍窗口不会变。
        //
        //  【为什么不直接改 MemberSlot】
        //  Group.h:364  _getMemberWSlot()  是 protected(358段)，外部够不着。
        //  Group.h:256  GetMemberSlots()   返回的是 const 引用，改不了。
        //
        //  【所以走官方路径：踢出 + 重加】
        //  Group.h:210  RemoveMember(ObjectGuid, ...)  public(185段)
        //               Group.cpp:695 有 guid.IsCreature() 专门分支，支持bot
        //  Group.h:197  AddMember(Creature*)           public(185段)
        //               Group.cpp:491 重新拍一次快照，这次拿到新名字
        RefreshGroupName(handler, entry);
