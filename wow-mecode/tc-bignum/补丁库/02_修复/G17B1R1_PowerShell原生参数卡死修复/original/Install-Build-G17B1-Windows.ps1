...
$UploadDir=Join-Path $Workspace "uploads";$Result=Join-Path $UploadDir "G17B1_WINDOWS_BUILD_RESULT.txt";$Utf8NoBom=New-Object System.Text.UTF8Encoding($false)
...
function Native([string]$File,[string[]]$Args,[string]$Prefix){$old=$ErrorActionPreference;$out=@();$rc=9009;try{$ErrorActionPreference="Continue";$out=@(& $File @Args 2>&1);$rc=$LASTEXITCODE}finally{$ErrorActionPreference=$old};foreach($line in $out){W ($Prefix+"|"+$line.ToString())};return [int]$rc}
function FindPython(){
...
try{
 W "G17B1_WINDOWS_BUILD_START";W "SCOPE=G17B1_ALL_OWNED_DIRECT_MOUNT_AUTO_INTERCEPT_AND_TYPED_SESSION";W "RUNS_SQL=False";W "MODIFIES_CLIENT=False";W "R5_CLIENT_STATE_MODIFIED=False"
 ...
 $com=$env:ComSpec;if(-not $com){throw "ComSpec missing"};$rc=Native $com @('/d','/c','echo G17B1_NATIVE_STDOUT& echo G17B1_NATIVE_STDERR 1>&2& exit /b 0') 'NATIVE_SELFTEST';W "NATIVE_SELFTEST_EXIT=$rc";if($rc-ne 0){throw "native runner selftest failed"}
 $rc=Native $python @($Tool,'apply','--source-root',$SourceRoot) 'SOURCE_APPLY';W "SOURCE_APPLY_EXIT=$rc";if($rc-ne 0){throw "source apply failed"}
 ...
 $rc=Native $msbuild @($Solution,'/m','/t:worldserver','/p:Configuration=RelWithDebInfo','/p:Platform=x64','/verbosity:minimal') 'MSBUILD';W "MSBUILD_EXIT=$rc";if($rc-ne 0){throw "MSBuild failed"}
 ...
 W "G17B1_WINDOWS_BUILD_RESULT=PASS";W "STOP_DO_NOT_RUN_SQL";W "NEXT=Start worldserver normally; use normal owned mount buttons with default auto=on";W "RESULT_FILE=$Result";exit 0
}catch{W ("G17B1_WINDOWS_BUILD_ERROR="+$_.Exception.Message);W "G17B1_WINDOWS_BUILD_RESULT=FAIL";W "STOP_DO_NOT_START_WORLDSERVER";W "RESULT_FILE=$Result";exit 1}
