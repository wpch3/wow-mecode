...
public:
    ...
    static bool DoPin(ChatHandler* handler, Player* player)
    {
        ...
        // ---- step63: 新判断模型，以 IsWandererBot() 为权威 ----
        ...
        {
            snprintf(buf, sizeof(buf),
                     "|cffff0000 这个bot已被玩家(guid=%u)招募，不能固定|r", owner);
            handler->SendSysMessage(buf);
            handler->SendSysMessage("|cffffff00 招募来的bot本来就是永久的|r");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->SendSysMessage("|cffff0000 数据损坏：creature表有记录但内存无数据|r");
            handler->SendSysMessage("|cffff0000 这种状态下次重启会崩服，请立刻 .pin fix|r");
            handler->SetSentErrorMessage(true);
            return false;
        }

        // st == PIN_STATE_WANDERING -> 放行
