...
- **G17-B1已关闭；B2真实部署后体验验收FAIL，当前唯一动作是B2R1。** 用户确认B2技能3向前/向上与控制恢复PASS，但否决技能2高速反馈、高速转向锐角折线及技能4仍有降落伞/类型姿态错误。B2R1新增技能2启动/持续/极速/结束四阶段反馈；技能3改为7节点Catmull-Rom曲线、偏航上限0.70弧度、三级LOS回退和末端切线平滑交还；技能4迁移到真实项目DBC中无视觉/无Aura的52226，并按魔法平姿、龙45码斜坡、机械反推、猛兽无火扑落和通用避障分段着陆。源码=`8b47a5b...c1d5 -> ff185d99...c4fc`，安全回滚=`e298a856...0203`；payload与安全回滚GCC14/C++20零诊断，52/52、未知SHA零写入拒绝、World迁移三状态模型、双AST、18文件包及ZIP复验PASS。唯一包`G17B2R1_Runtime_Experience_Rework_Windows_20260824.zip`（73526字节，SHA=`94822d39...8daa`）已由用户真实执行：源码应用、World迁移、MSBuild、新鲜OBJ/EXE/PDB和新EXE哈希全部PASS，禁止重跑；当前只待启动新worldserver后的客户端Runtime。B1/B2旧包与客户端R1–R5禁止重跑。
