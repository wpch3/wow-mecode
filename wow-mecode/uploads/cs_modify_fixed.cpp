...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "allstats",     rbac::RBAC_PERM_COMMAND_MODIFY_ALLSTATS,     false, &HandleModifyAllStatsCommand,      "" },
            { "stat",         rbac::RBAC_PERM_COMMAND_MODIFY_STAT,         false, &HandleModifyStatCommand,          "" },
    ...
    static bool HandleModifyStatCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (statName == "reset")
        {
            for (uint8 i = STAT_STRENGTH; i < MAX_STATS; ++i)
            {
                 ...
                 * step20 关键：必须同步清库！
                 *
                 * 只清内存的话，.modify stat reset 当场看着是重置了，
                 * 但库里记录还在 —— 【重登又全冒出来】。
                 * Record(..., 0) 表示删除该项（CustomStatPersist.h:86）。
                 */
                if (sCustomStatPersist->Enabled())
                    sCustomStatPersist->Record(target->GetGUID().GetCounter(),
