...
1. **当前主线：B3R10 用户验证成功（"八个御龙术技能已经出来了"）——御龙 UI 目标达成（C11 客户端魔改＋B3R10 八格布局全链路闭环）。** 本批交付＝F45R1（拾取播报源码级静默，conf 无效的根治）＋B3R11（陆地坐骑飞行蹬腿修复：BEAST/GENERIC 原型 EMOTE_STATE_STAND 冻结），待用户安装。下一阶段已立项：技能差异化（五形态设计文档就绪）。
...
| C13 客户端 DBC 三合一 | ①追加 990029/990030 ②飞行视觉 ③25 战斗技冲击特效 | COMPLETE | 49870+2 | ✅ 用户验收（第7格✅飞行特效✅；战斗特效部分错乱→C14） |
| **C14 客户端特效真相** | ①封禁 222（=Learning 483 升级金光）②槽位2 TargetA 18→1＋治疗/护盾特效 ③990029 更名裂地重击 | 内容验证 COMPLETE | — | 🟡 待验收（28/28） |
...
| c5c4c332 | B3R11 r1a 命名空间修复 | ✅ 用户验收（编译 PASS＋蹬腿修复生效） |
| 3d501d9b→a6074cde | B3R12 r1→r12a | ✅ 用户验收（无目标✅蹬腿✅入框✅第7格✅） |
| **c051fcda** | **B3R13 更名＋休憩＋原型测试** | **当前 payload；裂地重击聊天更名＋.dragon rest 御龙休憩（B4第一步：骑龙悬停每秒回5%血蓝，移动取消）＋.dragon archetype 0-4 实时切族测试；PRE=r12a；回滚 B3R6；48/48** | **当前 payload；CheckCast 目标预检（完整链）＋990029 突袭（首个AoE）＋990030 御风姿态＋第7格页面纯粹化；PRE=c5c4c332；回滚＝3fdb46e8；18 镜像；38/38 自检** |

安装器升级白名单＝上表除 2ddf54a6 外全部镜像；未知现场状态零写入拒绝。当前权威服务端包：仓库根 `G17B3R12_FINAL.zip`（payload 3d501d9b，38/38 包自检；包目录 `G17B3R12_无目标冷却修复_技能差异化/`；一条龙：源码→服务端 DBC 追加 990029/990030→world SQL 绑定→MSBuild）。客户端魔改：`G17C12_FINAL.zip`（SetScale 0.75，用户报告未生效待排查——疑似 SetSkin 重置或未重启，见 00-入口排查注）。F45R1 源码修改已就位但**从未进入 exe**（B3R11 编译失败），B3R12 编译成功后自动生效。B3R6-R11 关闭禁重跑。
