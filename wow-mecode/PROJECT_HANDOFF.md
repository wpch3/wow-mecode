...
4. **C9 v2 是当前唯一待执行动作**：Wowhead 逐条验证的 25 个真实法术视觉（每原型每槽独立）＋ DBC RecoveryTime=0。装完验收：5 技特效各不相同且符合名字语义、无目标按技能不再出现冷却转圈。
...
| **C9 v2** | **Wowhead 逐条验证 25 视觉＋RecoveryTime/Category=0** | 安装后新哈希 | 49868 | 🟡 已交付待用户安装验收 |

当前权威客户端包：仓库根 `G17C9_FINAL.zip`（SHA256 `e1bb16d07783883caff320cd0ba7e9305326399e21ec6f62b9f2157febfcb87e`）。前置：C3v2 已装（满足）。操作：关 worldserver＋WoW → 双击 `01_Install_G17C9.cmd` → PASSED → 重启两端。
...
### P0（当前唯一执行）：G17-C9 v2 用户安装与终验
- 包：`G17C9_FINAL.zip`（根目录，SHA256 `e1bb16d0...fcb87e`）。前置 C3v2 已满足。
...
- C 系（客户端 DBC）：C1（52226 门）、C2（+25 技能）、C3v2（+4 按钮，修复 v1 跑旧包问题）、C6（类型视觉＋射程）、C7（DUMMY→SCHOOL_DAMAGE）、C8（🔴 视觉猜错＋幻影冷却，被 C9 取代）、**C9 v2（🟡 待装）**。
...
| `wow-mecode/tc-bignum/00-当前整体安装步骤_单文件入口.md` | 顶部插入 2026-08-27 最新批次指针（C9 v2＋B3R6 为当前权威包） |

本轮**不改任何服务端源码、SQL、客户端文件**；纯交接同步批。
...
4. 若用户已装 C9：收取验收结论（特效四点）。PASS → 进入技能差异化设计＋B4；个别槽不符 → 仅该槽查 Wowhead 换视觉重打 C9（幂等）。
5. 若用户未装 C9：引导双击 `01_Install_G17C9.cmd`，等 PASSED 后进游戏验收。
...
- **C9 v2 用户安装与游戏内验收结果**（当前最大未知；特效是否符合语义、幻影冷却是否根除）
...
# 当前权威交付包（仓库根，用户从这里拿）
G17C9_FINAL.zip        # 客户端：Wowhead视觉+清DBC冷却（待装）SHA e1bb16d0...fcb87e
...
G17C9_真实特效_客户端/G17C9_Real_Visuals_Fix_Client_20260826/ # tools/patch_g17c9.py=视觉真值表
...
2. G17 御龙术服务端链（多页技能栏/25 技能/能量/自动战斗/性能）与客户端链（按钮/射程/特效渲染/冷却）均已修到各自的最新交付：服务端 B3R6（3fdb46e8）、客户端 C9 v2（Wowhead 验证视觉＋零 DBC 冷却）。**唯一待办＝C9 用户验收。**
