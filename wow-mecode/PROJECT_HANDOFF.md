...
1. **当前主线：C11 客户端魔改用户验证成功（原版条 6→7 格、切页@6 可用、退出正常）——客户端魔改路线实战闭环。** 用户指令：切页移到第 8 格＋继续势头 → 当前交付＝B3R10（八格全满：切页恒@8、制动恒@6、生成器/俯冲@7、切页聊天反馈），待用户安装验收。G22 深度客户端魔改能力已由 C11 实战证明：提取→改 FrameXML→MPQ 下发→生效。
...
| **199cff4a** | **B3R10 八格全满＋切页@8** | **当前 payload；移动页[5机动|制动@6|生成器@7|切页@8]、战斗页[技1-5|制动@6|俯冲@7|切页@8]＋切页/上龙聊天反馈；PRE=B3R8 dcfa78dd；回滚＝3fdb46e8；B3R6-R9 均可直装（33/33 自检）** |

安装器升级白名单＝上表除 2ddf54a6 外全部镜像；未知现场状态零写入拒绝。当前权威服务端包：仓库根 `G17B3R10_FINAL.zip`（payload 199cff4a，33/33 包自检 PASS）；包目录 `tc-bignum/规划/G17_飞行与移动/G17B3R10_八格全满_切页第八格/G17B3R10_EightSlots_SwitchAt8_Windows_20260827/`。B3R6-R9 包关闭禁重跑（B3R10 已包含其全部改进）。
